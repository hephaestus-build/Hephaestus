...
name: human-copy
description: >
  Write or review copy so it reads as written by a person. Use when writing or changing a
  user-facing string under `webapp/src/**`, a page under `docs/`, a changeset in `.changeset/`,
  a pull request body, or Heph's lines, and when reviewing a diff for AI-writing patterns:
  not-X-but-Y contrasts, forced triads, dashes, semicolons, middle dots, colon-led reveals,
  stacked hedges, participial tails, mirrored clauses. Vendored from blader/humanizer, with
  house rules for Hephaestus on top.
license: MIT
metadata:
  author: blader
  version: "3.0.0"
...
Read these first. They win over the vendored rules below where the two disagree; the vendored
rules below are upstream's and are not rewritten here. `SOURCE.md` names the upstream version.

- British spelling: organisation, colour, customise, licence as a noun.
- The product vocabulary in `docs/contributor/practice-feedback-language.md` wins over any rewrite.
  Never swap a product term for a synonym to escape a pattern below.
- Never a semicolon, a middle dot (·) or an em dash in UI copy. Use a full stop or a plain word.
  §8 below allows a dash when a writing sample uses one; here there is no exception, and the ban
  covers the semicolon and the middle dot too.
- Heph speaks about Hephaestus in the third person ("Hephaestus reads your work…"), not "I".
- Buttons are verbs: Save, Connect, Reload, Skip for now. Never Next, Finish, Submit, Done or OK.
- One idea per sentence, about 20 words. Split anything longer.
- Contractions are fine.
- No "please".

For a short string (a button, a hint, an alert title, a card tagline) the patterns that bite most
are §1, §2, §6, §8, §9, §15 and §19. A colon that introduces a reveal ("One question: which AI…")
is §19 in a sentence; state the point instead. In review, list each hit as file, string, pattern
number, rewrite.
