...
async function posixGlob(pattern: string): Promise<string[]> {
	...
	return files.map((file) => file.split(path.sep).join("/")).toSorted();
}
...
function invokedTasks(definition: YAMLMap): string[] {
	...
	for (const item of steps.items) {
		if (!isMap(item) || typeof item.get("run") !== "string") {
			continue;
		}
		// A command a step prints as guidance is not a command it runs.
		const command = String(item.get("run")).replaceAll(/`[^`]*`/gu, "");
		for (const invocation of command.matchAll(TASK_INVOCATION)) {
			const raw = invocation.groups?.name;
			// `vp run --filter <package> <script>` runs a package script rather than a task.
			if (raw === undefined || raw.startsWith("-")) {
				continue;
			}
			const expression = /\$\{\{ *matrix\.(?<key>\w+) *\}\}/u.exec(raw);
			const key = expression?.groups?.key;
			if (expression === null || key === undefined) {
				names.push(raw);
				continue;
			}
			const values = isMap(matrix) ? matrixValues(matrix, key) : [];
			assert.ok(values.length > 0, `matrix.${key} has no values`);
			for (const value of values) {
				names.push(raw.replace(expression[0], value));
			}
		}
	}
	return names;
...
async function importClosure(entry: string): Promise<string[]> {
	...
	while (queue.length > 0) {
		...
		for (const { groups } of source.matchAll(/from\s+"(?<specifier>\.[^"]+\.ts)"/gu)) {
			queue.push(path.posix.normalize(path.posix.join(directory, groups?.specifier ?? "")));
		}
		for (const { groups } of source.matchAll(/"(?<name>[\w.-]+\.ts)"/gu)) {
			const candidate = path.posix.normalize(path.posix.join(directory, "..", groups?.name ?? ""));
			if (candidate.startsWith("scripts/") && existsSync(candidate)) {
...
function exceptionFault(entry: unknown, now: number): string | undefined {
	...
	if (typeof entry.id !== "string" || entry.id.trim() === "") {
		return "names no vulnerability id";
	}
	if (
	...
	if (typeof entry.expired_at !== "string" || !/^\d{4}-\d{2}-\d{2}$/u.test(entry.expired_at)) {
 ...
 */
function runnerStepsSkipReason(): string | false {
	if (!bashRunsRunnerSteps()) {
		return "this bash has no mapfile; runner steps need bash 4 or later";
	}
	if (process.platform === "win32") {
		return "the buildpack step runs only on Linux runners; PATH stubs are not reliably executable here";
	}
	return false;
}

const runnerStepsOnly = { skip: runnerStepsSkipReason() };
...
function render(
	...
	const value = (operand: string): string => {
		const result = /^needs\.(?<job>[\w-]+)\.result$/u.exec(operand)?.groups?.job;
		if (result !== undefined) {
			const conclusion = results[result];
			assert.ok(conclusion !== undefined, `${operand} is not one of the job's needs`);
			return conclusion;
		}
		const output = /^needs\.detect-changes\.outputs\.(?<name>[\w-]+)$/u.exec(operand)?.groups?.name;
		if (output !== undefined) {
			const declared = outputs[output];
			assert.ok(declared !== undefined, `${operand} is not a detect-changes output`);
			return declared;
		}
		const contained = /^contains\(needs\.\*\.result, '(?<result>\w+)'\)$/u.exec(operand)?.groups
			?.result;
		if (contained !== undefined) {
			return String(Object.values(results).includes(contained));
		}
		throw new Error(`this test cannot evaluate \`${operand}\``);
	};
	return shell.replaceAll(/\$\{\{ (?<expression>.+?) \}\}/gu, (_, expression: string) => {
		const terms = expression.split(" && ");
		if (terms.length === 1 && !/ (?:==|!=) /u.test(expression)) {
		...
		return String(
			terms.every((term) => {
				const comparison = /^(?<operand>.+) (?<operator>==|!=) '(?<literal>\w*)'$/u.exec(term);
				assert.ok(comparison, `this test cannot evaluate \`${term}\``);
				const { operand, operator, literal } = comparison.groups ?? {};
...
function gates(names: Set<string>): string[] {
	return [...names].filter((name) => name.startsWith("gate:")).toSorted();
}
...
function taskClosure(tasks: Record<string, unknown>, roots: Iterable<string>): Set<string> {
	...
	const expand = (name: string): void => {
		...
		for (const command of commandsOf(task)) {
			const nested = /^vp run (?<task>[\w:-]+)$/u.exec(command)?.groups?.task;
			if (nested !== undefined) {
				expand(nested);
			}
		}
...
void describe("CI contract", () => {
	...
	void test("task names follow the vocabulary in AGENTS.md", async () => {
		...
		for (const name of Object.keys(tasks).toSorted()) {
			assert.match(name, /^[a-z]+(?::[a-z0-9-]+)*$/u, `${name} is not a valid task name`);
	...
	void test("server CI uses native Gradle caching and verifies JUnit selection", async () => {
		...
		assert.match(action, /gradle\/actions\/setup-gradle@/u);
		assert.doesNotMatch(action, /cache-type|\.m2/u);
	...
	void test("the server package job is the only Gradle cache producer", async () => {
		...
		assert.match(source, /inputs.cache-write != 'true' \|\| github.ref != format/u);
	});
	...
	void test("image scans are delegated only to required release preflight", async () => {
		...
		assert.match(
			...
			/inputs.scan-images && inputs.publish/u,
		);
	});
		...
		for (const [file, source] of await workflowSources()) {
			visit(parseDocument(source), {
				Map(_key, declaration) {
					if (!String(declaration.get("uses")).startsWith("dorny/test-reporter@")) {
						return;
					}
	...
	void test("runs the integration tier once, under the status gate, off an untemplated command", async () => {
		...
		assert.match(integration, /fail-fast: false/u, "One failing shard must not hide the other");
		...
		assert.equal((source.match(/run: vp run test:server:integration/gu) ?? []).length, 1);
		assert.match(
			integration,
			/Test Results - App Server Integration \(\$\{\{ matrix.shard \}\}\)/u,
		);
		const tasks = await readFile("vite.config.ts", "utf8");
		assert.match(
			await readFile("server/application/build.gradle.kts", "utf8"),
			/HEPHAESTUS_INTEGRATION_SHARD/u,
		);
		assert.match(tasks, /"test:server:integration": run\([\s\S]*?:application:integrationTest/u);
		const orchestrator = await readFile(".github/workflows/cicd.yml", "utf8");
		assert.match(job(orchestrator, "Test"), /uses: \.\/\.github\/workflows\/ci-tests.yml/u);
		assert.match(job(orchestrator, "all-ci-passed"), /needs\.Test\.result/u);
	});
	...
	void test("packages the server once and runs every artifact gate against it", async () => {
		...
		for (const name of ["server-api", "server-database"]) {
			...
			assert.doesNotMatch(
				consumer,
				/gradlew[^\n]* :(?:application|generated-clients):(?:compileJava|compileTestJava|bootJar|classes|testClasses)(?:\s|$)/u,
			);
		}
		...
		assert.doesNotMatch(tests, /^ {4}needs:|download-artifact|restore-server-build/mu);
		assert.match(job(tests, "server-verification"), /vp run test:server:verification/u);
		assert.match(job(tests, "server-integration"), /vp run test:server:integration/u);
		assert.match(job(orchestrator, "Build"), /needs: \[detect-changes, server-package\]/u);

		const reusable = await readFile(".github/workflows/reusable-docker-build.yml", "utf8");
		const packBuilds = [
			...reusable.replaceAll(/\\\n\s*/gu, " ").matchAll(/^\s+pack build .*$/gmu),
		].map((match) => match[0]);
		...
		for (const [file, source] of await workflowSources()) {
			assert.doesNotMatch(source, /bootBuildImage/u, `${file} must build the image from the JAR`);
		}
	});
	...
	void test("builds Storybook once and gives its TurboSnap stats to Chromatic", async () => {
		...
		assert.equal((storybook.match(/vp run --filter webapp build-storybook/gu) ?? []).length, 1);
		assert.match(storybook, /test -s webapp\/storybook-static\/preview-stats\.json/u);
		assert.match(storybook, /storybookBuildDir: storybook-static/u);
		assert.match(storybook, /onlyChanged: true/u);
		assert.match(storybook, /surge \.\/webapp\/storybook-static/u);
		assert.match(storybook, /render-preview-comment\.ts storybook webapp\/storybook-static/u);
		assert.match(storybook, /github\.event\.pull_request\.base\.sha/u);
		assert.match(
			storybook,
			/PREVIEW_COMMENT_PATH: \$\{\{ runner\.temp \}\}\/storybook-preview\.json/u,
		);
		assert.match(storybook, /name: Create Storybook status check\s+if: >-\s+success\(\)/u);
	});
	...
	void test("publishes documentation links derived from the built pages", async () => {
		...
		assert.match(buildPreview, /fetch-depth: 0/u);
		assert.match(buildPreview, /render-preview-comment\.ts docs docs\/\.docusaurus/u);
		assert.match(buildPreview, /github\.event\.pull_request\.base\.sha/u);
		assert.match(
			job(docs, "preview"),
			/PREVIEW_COMMENT_PATH: preview-comment\/docs-preview-comment\.json/u,
		);
	});
	...
	void test("shares a continuation-aware publisher across both previews and teardown", async () => {
		...
		assert.match(teardown, /scripts\/publish-preview-comments\.ts/u);
		assert.match(teardown, /for \(const kind of \["docs", "storybook"\]\)/u);
		assert.match(teardown, /publishPreviewComments\(\{ github, context, kind \}\)/u);
	});
	...
	void test("routes tooling-only changes away from server infrastructure", async () => {
		...
		for (const pattern of ["docs/**", ".vscode/settings.json", "**/AGENTS.md", "**/CLAUDE.md"]) {
			assert.match(tooling, new RegExp(`- '${escapeRegExp(pattern)}'`, "u"));
		}
	...
	void test("invalidates CI legs through their owned workflow dependencies", async () => {
		...
		for (const [filter, workflow] of expected) {
			...
			assert.match(patterns, new RegExp(`- '${escapeRegExp(workflow)}'`, "u"));
			assert.match(patterns, /- '\.github\/workflows\/cicd\.yml'/u);
			assert.doesNotMatch(patterns, /- '\.github\/workflows\/\*\*'/u);
			assert.match(source, new RegExp(`outputs\\.${escapeRegExp(filter)} == 'true'`, "u"));
		}
	});
	...
	void test("separates pre-merge validation from post-merge artifact production", async () => {
		...
		for (const event of ["workflow_dispatch", "push", "pull_request", "merge_group"]) {
			assert.match(source, new RegExp(`^ {2}${event}:`, "mu"));
		}
		assert.match(
		...
		for (const name of ["workflow-lint", "zizmor", "Quality", "Security", "Test", "Compose"]) {
			...
			assert.match(
				job(source, name),
				/github\.event_name != 'push'.*needs\.detect-changes\.outputs\.version-bump == 'true'/su,
			);
		}
		...
		assert.match(
			job(source, "Compose"),
			/uses: \.\/\.github\/workflows\/ci-compose-validate\.yml/u,
		);
	});
	...
	void test("reports a failed merge-group run on its pull request, and only a failure", async () => {
		...
		assert.match(reporter, /needs: \[all-ci-passed\]/u);
		assert.match(reporter, /github\.event_name == 'merge_group'/u);
		// A merge queue cancels the runs it regroups, which says nothing about the pull request.
		assert.match(reporter, /needs\.all-ci-passed\.result == 'failure'/u);
		assert.doesNotMatch(reporter, /always\(\)/u);
		assert.match(reporter, /pull-requests: write/u);
		// A ref the job cannot read is a missing report, so it fails rather than commenting nowhere.
		assert.match(reporter, /core\.setFailed/u);
	});
	...
	void test("a merge group selects its jobs by the paths its own diff touches", async () => {
		...
		for (const name of [
			...
			assert.match(
				...
				/needs\.detect-changes\.outputs\.unfiltered == 'true'/u,
		...
		// the event.
		const build = await readFile(".github/workflows/ci-build.yml", "utf8");
		assert.doesNotMatch(build, /github\.event\.pull_request\.base\.sha/u);
	});
	...
	void test("decides an image's architectures once, for every image the run builds", async () => {
		...
		for (const secret of ["SENTRY_AUTH_TOKEN", "SENTRY_ORG", "SENTRY_PROJECT"]) {
			assert.match(
				caller,
				new RegExp(
					`${secret}: \\$\\{\\{ github\\.event_name == 'push' && secrets\\.${secret}`,
					"u",
				),
			);
		}
		...
		const reusable = await readFile(".github/workflows/reusable-docker-build.yml", "utf8");
		for (const secret of ["SENTRY_AUTH_TOKEN", "SENTRY_ORG", "SENTRY_PROJECT"]) {
			assert.match(reusable, new RegExp(`github\\.event_name == 'push'.*secrets\\.${secret}`, "u"));
		}
		assert.match(
			reusable,
			/inputs\.single-arch.*linux\/amd64.*ubuntu-24\.04.*linux\/arm64.*ubuntu-24\.04-arm/u,
		);
	});
	...
	void test("a pull request based on an unmerged branch still reaches a Docker verdict", async () => {
		...
		assert.match(
			...
			/^\$\{\{ steps\.alias_base\.outputs\.commit \}\}$/u,
		);
		...
		assert.equal(
			await resolveAliasBase("b".repeat(40), "main", {
				compare: async (base) => (base === published ? "ahead" : "diverged"),
				baseOf: async () => published,
			}),
			published,
		);
		...
		assert.match(
			...
			/^\$\{\{ needs\.detect-changes\.outputs\.alias-base \}\}$/u,
		);
		const shell = runScript(docker, tagging, "Verify and tag unchanged images");
		assert.match(shell, /\$ALIAS_BASE/u);
		assert.doesNotMatch(shell, /BASE_SHA/u);
		...
		for (const image of [
			...
			assert.match(
				...
				new RegExp(`inputs\\.${image} != 'true'`, "u"),
			);
		}
	});
	...
	void test("builds fork pull-request images without registry writes", async () => {
		...
		for (const caller of ["application-server-image", "Docker"]) {
			assert.match(
				...
				/^\$\{\{ needs\.detect-changes\.outputs\.publishable == 'true' \}\}$/u,
			);
		}
		...
		assert.match(
			...
			/^\$\{\{ inputs\.use-buildpacks && 'docker' \|\| 'docker-container' \}\}$/u,
		);
		assert.equal(
		...
		assert.match(String(inputs.get("push")), /^\$\{\{ inputs\.publish \}\}$/u);
		// An untagged local image is one nothing can name, so a fork only loads what it also tags.
		assert.match(
			String(inputs.get("load")),
			/^\$\{\{ !inputs\.publish && inputs\.single-arch \}\}$/u,
		);
		assert.match(String(inputs.get("outputs")), /inputs\.publish &&/u);
		for (const gated of ["merge", "scan"]) {
			assert.match(String(reusable.getIn(["jobs", gated, "if"])), /inputs\.publish/u);
		}

		const docker = parseDocument(await readFile(".github/workflows/ci-docker-build.yml", "utf8"));
		assert.match(String(docker.getIn(["jobs", "tag-unchanged-images", "if"])), /inputs\.publish/u);
	});
	...
	void test(
		...
		async () => {
			...
			assert.match(local.outputs.digest ?? "", /^sha256:/u);
			const invoked = await readFile(calls, "utf8");
			assert.doesNotMatch(invoked, /--publish/u);
			assert.match(invoked, /^pack build .*--trust-builder/mu);
			...
			for (const entry of jobs.items) {
				...
				const boots = steps.items.some(
					(item) =>
						...
						/docker compose .*--env-file[\s\S]*?\bup -d\b/u.test(String(item.get("run"))),
				);
				...
				assert.match(
					job(source, name),
					/uses: \.\/\.github\/actions\/ghcr-login/u,
	...
	void test("a pull request boots the supported installation from the images its own run built", async () => {
		...
		assert.match(condition, /needs\.detect-changes\.outputs\.previews == 'true'/u);
		assert.match(
			String(orchestrator.getIn(["jobs", "detect-changes", "outputs", "previews"])),
			/github\.event_name == 'pull_request' && github\.event\.pull_request\.head\.repo\.full_name == github\.repository/u,
		);
		assert.match(condition, /needs\.detect-changes\.outputs\.supported-host-smoke == 'true'/u);
		...
		assert.match(
			...
			/scripts\/prepare-host-smoke-env\.ts/u,
		);
	...
	void test("the supported-host boot smoke runs on every source it can be broken by", async () => {
		...
		const patterns = [...filter.matchAll(/^ +- '(?<pattern>.+)'$/gmu)].map(({ groups }) =>
			String(groups?.pattern),
		);
		const globbed = await Promise.all(patterns.map(posixGlob));
		const selected = new Set(globbed.flat());
		...
		for (const file of await posixGlob("server/application/src/main/java/**/*.java")) {
			if (!/^@ConfigurationProperties(?:Scan)?\b/mu.test(await readFile(file, "utf8"))) {
				continue;
			}
			assert.ok(
				selected.has(file),
	...
	void test("every image a consumer resolves is one the build published for that event", async () => {
		...
		const guard =
			/^type=raw,value=\$\{\{ (?<value>.+?) \}\},enable=\$\{\{ github\.event_name (?<operator>==|!=) '(?<event>\w+)' \}\}$/u;
		...
		const publishedOn = (event: string): string[] =>
			lines.flatMap((line) => {
				...
				const { value, operator, event: guarded } = parsed.groups ?? {};
				return (operator === "==") === (guarded === event) ? [String(value)] : [];
			});
		...
		assert.doesNotMatch(String(reusable), /steps\.tags\.outputs|Prepare tag configuration/u);

		const source = await readFile(".github/workflows/cicd.yml", "utf8");
		const triggers = /^on:\n(?<body>[\s\S]*?)^\S/mu.exec(source)?.groups?.body ?? "";
		const events = [...triggers.matchAll(/^ {2}(?<event>\w+):/gmu)].map(({ groups }) =>
			String(groups?.event),
		);
		...
		assert.match(
			preflight,
			/COMMIT: \$\{\{ github\.event\.pull_request\.head\.sha \|\| github\.sha \}\}/u,
		);
		assert.match(preflight, /resolve-release-images\.ts "\$COMMIT"/u);
		assert.match(preflight, /--commit "\$COMMIT"/u);
		const smoke = job(source, "Supported-host-smoke");
		assert.match(smoke, /HEAD_SHA: \$\{\{ github\.event\.pull_request\.head\.sha \}\}/u);
		// `workflow_run.head_sha` is the producing push run's own `github.sha`.
		const release = await readFile(".github/workflows/release.yml", "utf8");
		assert.match(
			job(release, "tag-images"),
			/SOURCE_TAG: \$\{\{ github\.event\.workflow_run\.head_sha \}\}/u,
		);
	...
	void test("enforces the release vulnerability policy where images are built", async () => {
		...
		assert.match(scan, /needs: \[build, merge\]/u);
		assert.match(scan, /!cancelled\(\)/u);
		assert.match(scan, /needs\.build\.result == 'success'/u);
		...
		assert.match(
			scan,
			/node scripts\/check-release-vulnerabilities\.ts[\s\S]*security\/vulnerability-policy\.json/u,
		);
		assert.match(scan, /needs\.build\.outputs\.manifest-digest/u);
		assert.match(scan, /needs\.merge\.outputs\.manifest-digest/u);
		...
		for (const [file, source] of await workflowSources()) {
			for (const call of source.match(
				/node scripts\/check-release-vulnerabilities\.ts[\s\S]*?\.policy\.json"/gu,
			) ?? []) {
				callSites += 1;
				assert.match(
					call,
					/(?<![\w./-])security\/vulnerability-policy\.json/u,
	...
	void test("keeps one release vulnerability policy behind every scan", async () => {
		...
		for (const [file, source] of sources) {
			...
			for (const reference of source.match(/[\w./-]*vulnerability-polic[\w-]*\.json/gu) ?? []) {
				assert.ok(
		...
		assert.match(
			...
			/copyFile\(\n?\s*"security\/vulnerability-policy\.json",/u,
		);
	...
	void test("bounds every captured subprocess above Node's 1 MiB default", async () => {
		...
		for (const file of await posixGlob("scripts/**/*.ts")) {
			...
			const captures =
				source.match(/exec(?:File)?Sync\(|execFileAsync\(|spawnSync\(/gu)?.length ?? 0;
			if (captures === 0 || !source.includes("encoding")) {
				continue;
			}
			if (!source.includes("maxBuffer")) {
				offenders.push(file);
			}
		}
	...
	void test("scans both released platforms before the release, not just linux/amd64", async () => {
		...
		assert.match(source, /for \(const platform of PLATFORMS\)/u);
	});
	...
	void test("performs every release evidence check that does not need a release before the release", async () => {
		...
		for (const source of [job(release, "tag-images"), preflight]) {
			assert.match(source, /node scripts\/resolve-release-images\.ts /u);
			assert.match(source, /node scripts\/generate-release-evidence\.ts evidence \\/u);
			assert.match(
				source,
	...
	void test("merge groups require release evidence when their aggregate tree changes the version", async (context) => {
		...
		const unknownBase = await detect("0".repeat(40));
		assert.notEqual(unknownBase.status, 0);
		await writeFile(path.join(directory, "package.json"), "{}");
		const unversioned = await detect(base);
	...
	void test("fails the CI gate on the Version PR when its release evidence preflight did not", async () => {
		...
		const detected = async (head: string): Promise<string | undefined> => {
			const result = await runStep(detect, { HEAD_BRANCH: head, MERGE_BASE_SHA: "" });
			return result.outputs["release-candidate"];
		};
		assert.equal(await detected(branch), "true");
		for (const other of [
			"main",
			`${branch}-old`,
			"changeset-release/release-1.0",
			"renovate/vite",
		]) {
		...
		assert.match(
			...
			/needs\.detect-changes\.outputs\.release-preflight == 'true'/u,
		);
		assert.match(
			String(workflow.getIn([...detection, "outputs", "should_skip"])),
			/steps\.release_candidate\.outputs\.release-candidate != 'true'/u,
		);
		...
		for (const preflight of ["skipped", "failure", "cancelled"]) {
			const result = await verdict({ "Release-preflight": preflight }, true);
			assert.equal(result.failed, true, `preflight ${preflight} must fail the gate`);
		}
		...
		const preMerge = new Set([shape.groups?.first, shape.groups?.second]);
		const bothPlatforms = (event: string, onVersionBranch: boolean): boolean =>
			!preMerge.has(event) || onVersionBranch;
		const events = ["pull_request", "merge_group", "workflow_dispatch", "push"];
		for (const onVersionBranch of [true, false]) {
			const result = await verdict({ "Release-preflight": "skipped" }, onVersionBranch);
			const demanded = result.failed;
			for (const event of events) {
				assert.ok(
		...
		assert.match(
			complete,
			/^\$\{\{ steps\.alias_base\.outputs\.commit == '' \|\| steps\.release_candidate\.outputs\.release-candidate == 'true' \}\}$/u,
		);
		...
		const aliasing = [...resolves.matchAll(/github\.event_name == '(?<event>\w+)'/gu)].map(
			({ groups }) => String(groups?.event),
		);
		...
		for (const name of ["server-package", "application-server-image"]) {
			...
			assert.match(condition, /needs\.detect-changes\.outputs\.all-images == 'true'/u);
			assert.doesNotMatch(condition, /github\.event_name/u);
		}
		const imageInputs = {
			Docker: [
				"webapp_changed",
		...
		for (const [caller, inputs] of Object.entries(imageInputs)) {
			for (const name of inputs) {
				...
				assert.match(
					value,
					/needs\.detect-changes\.outputs\.all-images == 'true'/u,
		...
		for (const name of imageInputs.Docker) {
			assert.match(alias, new RegExp(`inputs\\.${name} != 'true'`, "u"));
		}
	...
	void test("rescans main's images weekly and reports drift to an issue, not a status", async () => {
		...
		assert.match(source, /^ {4}- cron: "\d+ \d+ \* \* 1"$/mu);
		assert.match(source, /workflow_dispatch:/u);
		assert.match(source, /group: rescan-main-images/u);
		// Writing an issue is the whole point; nothing else here needs a write.
		assert.match(source, /issues: write/u);
		assert.doesNotMatch(source, /contents: write|packages: write|id-token: write/u);
		...
		assert.match(source, /run: node scripts\/scan-main-images\.ts reports/u);
		assert.match(source, /run: node scripts\/report-vulnerability-drift\.ts reports/u);
		assert.match(source, /uses: \.\/\.github\/actions\/download-trivy-db/u);
		assert.match(source, /uses: actions\/upload-artifact@/u);
		...
		assert.doesNotMatch(source, /trivy image/u);
	});
	...
	void test("fetches the Trivy database through one action", async () => {
		...
		assert.match(action, /public\.ecr\.aws\/aquasecurity\/trivy-db/u);
		assert.match(action, /--download-db-only/u);
		assert.match(action, /--download-java-db-only/u);
	...
	void test("generates verified Gradle snapshots without giving pull request code write access", async () => {
		...
		assert.doesNotMatch(
			source,
			/: write|verification=off|write-verification-metadata|dependency-submission@/u,
		);
	...
	void test("submits dependency snapshot data without executing the originating checkout", async () => {
		...
		assert.doesNotMatch(
			source,
			/uses: actions\/checkout|uses: \.\/|run:.*(?:gradlew|node|bash|curl)/u,
		);
		assert.equal(
	...
	void test("blocks dependency regressions across the release trust boundary", async () => {
		...
		assert.match(securityConfig, /- '\.github\/dependency-review-config\.yml'/u);
		assert.match(
			securityConfig,
			/- 'security\/trivy-dependency-ignore\.yaml'/u,
		...
		assert.match(String(secretScan.get("version")), /^\d+\.\d+\.\d+@sha256:[a-f0-9]{64}$/u);
		assert.equal(
			secretScan.get("base"),
		...
		assert.match(
			...
			/^\$\{\{ steps\.dependency-policy\.outcome \}\}$/u,
		);
	...
	void test("pins every external action to a full commit SHA with a version comment", async () => {
		...
		for (const [file, source] of await readSources(
			...
			for (const [index, line] of source.split("\n").entries()) {
				const uses = /^\s+(?:- )?uses:\s+(?<reference>\S+)(?<comment>.*)$/u.exec(line);
				if (!uses) {
					continue;
				}
				const reference = uses.groups?.reference;
				const comment = uses.groups?.comment;
				assert.ok(reference !== undefined && comment !== undefined);
				if (reference.startsWith("./")) {
					continue;
				}
				if (!/@[0-9a-f]{40}$/u.test(reference) || !/^ # v\S+(?:\s.*)?$/u.test(comment)) {
	...
	void test("centralises dependency installation and browser setup", async () => {
		...
		for (const [file, source] of sources) {
			...
			visit(parseDocument(source), {
				Map(_key, node) {
					if (node.get("uses") !== "./.github/actions/setup-toolchain") {
						return;
					}
	...
	void test("creates the draft release only once the evidence gate has passed", async () => {
		...
		assert.doesNotMatch(decide, /gh release create/u);
		...
		assert.ok(gated !== -1, "tag-images must run the evidence verifier");
		...
		assert.ok(published !== -1 && immutable > published, "publication must verify immutable state");
		...
		for (const upload of source.matchAll(/gh release upload[\s\S]*?\n\n/gu)) {
			assert.match(upload[0], /--clobber/u);
		}
	});
	...
	void test("exempts a verified revert from the changeset freeze rules", async () => {
		...
		assert.ok(detect !== -1 && guard > detect, "the revert check must precede the freeze guard");
		assert.match(source, /run: node scripts\/verify-revert\.ts "\$BASE_SHA" HEAD/u);
		assert.match(source, /steps\.revert\.outputs\.verified-revert != 'true'/u);
		// The exemption is structural: a title or branch name is attacker-chosen and never read.
		assert.doesNotMatch(source, /pull_request\.title|github\.head_ref/u);
	});
	...
	void test("creates every automation commit through the API, so GitHub signs it", async () => {
		...
		for (const [file, source] of await workflowSources()) {
			for (const [index, line] of source.split("\n").entries()) {
				...
				const command = /\bgit push\b.*/u.exec(statement)?.[0];
				if (command === undefined || /\s--dry-run(?:\s|$)/u.test(command)) {
					continue;
				}
	...
	void test("checks out no ref taken straight from a pull_request_target or workflow_run event", async () => {
		...
		for (const [file, source] of await workflowSources()) {
			if (!/^ {2}(?:pull_request_target|workflow_run):/mu.test(source)) {
				continue;
			}
			assert.doesNotMatch(
				source,
				/^ +ref:.*github\.event\.(?:pull_request|workflow_run)/mu,
		...
		assert.match(
			...
			/git merge-base --is-ancestor "\$SHA" origin\/main/u,
		);
	});
	...
	void test("resolves every commit's author without checking out the pull request", async () => {
		...
		assert.match(identity, /ref: \$\{\{ github\.event\.repository\.default_branch \}\}/u);
		assert.match(identity, /uses: actions\/github-script@/u);
	});

	void test("never invokes a repository-local action before checkout", async () => {
		for (const [file, source] of await workflowSources()) {
			for (const jobSource of source.split(/^ {2}(?=[A-Za-z][\w-]*:\s*$)/mu).slice(1)) {
				const firstLocalAction = jobSource.indexOf("uses: ./.github/actions/");
				if (firstLocalAction === -1) {
					continue;
				}
				const checkout = jobSource.indexOf("uses: actions/checkout@");
				assert.ok(
					checkout !== -1 && checkout < firstLocalAction,
	...
	void test("bounds every job that runs steps of its own", async () => {
		...
		for (const [file, source] of await workflowSources()) {
			...
			for (const entry of jobs.items) {
				...
				if (!isMap(definition) || !isSeq(definition.get("steps"))) {
					continue;
				}
...
void test("the task graph keeps its cache posture", async () => {
	...
	for (const [name, task] of Object.entries(tasks)) {
		...
		if (task.cache !== false) {
			for (const command of commands) {
				assert.doesNotMatch(command, /\bvp run\b/u, `${name} must own its command to cache`);
				assert.doesNotMatch(command, /\bvp exec\b/u, `${name} cannot cache through vp exec`);
			}
		}
		// Shell parameter expansion is outside the runner contract (scripts/check-runner-contract.ts).
		for (const command of commands) {
			assert.doesNotMatch(command, /\$/u, `${name} must not rely on shell expansion`);
		}
	...
	assert.match(
		serverCommands[0] ?? "",
		/run-gradlew\.ts.*:spotlessCheck.*:application:spotlessCheck.*:application:pmdMain/u,
	);
	...
	for (const [file, source] of await workflowSources()) {
		assert.doesNotMatch(
			source,
			/^\s*(?:run:\s*)?(?:vp exec )?(?:vite|vitest|oxlint|oxfmt)(?:\s|$)/mu,
	...
	for (const [file, source] of await workflowSources()) {
		...
		for (const pair of jobs.items) {
			...
			for (const item of steps.items) {
				...
				if (typeof run === "string" && /\bvp install\b/u.test(run)) {
					installsItself = true;
				}
...
void test("the Scorecard ratchet runs on every event that can move a score, outside publishing", async () => {
	...
	assert.match(
		...
		/security\/scorecard-baseline\.json/u,
	);
	...
	for (const item of steps.items) {
		...
		assert.match(
			...
			/^(?:actions\/(?:checkout|upload-artifact)|github\/codeql-action\/upload-sarif|ossf\/scorecard-action)@/u,
		);
	}
});
...
void test("a workflow triggered by main does not cancel the run main is judged by", async () => {
	...
	for (const [file, source] of sources) {
		...
		if (
			!isSeq(branches) ||
			!branches.items.some((item) => isScalar(item) && item.value === "main")
		) {
			continue;
		}
		if (
...
void test("proves the toolchain on Windows and from a clean clone", async () => {
	...
	assert.match(quality, /runs-on: .*inputs\.leg == 'windows'.*'windows-latest'.*'ubuntu-latest'/u);
	assert.match(quality, /shell: bash/u);
	assert.match(quality, /windows\) vp run --no-cache ci:windows/u);
	for (const name of ["Restore Vite task cache", "Save Vite task cache"]) {
		assert.match(
			String(namedStep(workflow, qualityPath, name).get("if")),
			/inputs\.leg != 'windows'/u,
		);
	}
	const install = job(source, "clean-install");
	assert.doesNotMatch(install, /setup-toolchain|pnpm\/setup/u);
	assert.match(install, /vp install --frozen-lockfile/u);
	assert.match(install, /vp run gate:toolchain/u);
	...
	assert.match(commitHook, /node scripts\/enable-hooks\.ts/u);
	assert.match(commitHook, /git config core\.hooksPath/u);
	assert.equal((commitHook.match(/git commit --allow-empty/gu) ?? []).length, 2);
	const prePushHook = runScript(workflow, qualityPath, "Pre-push hook");
	assert.match(prePushHook, /node scripts\/enable-hooks\.ts/u);
	assert.match(prePushHook, /git push --dry-run\b/u);
	// The step moves the launcher aside, so it must put it back however it exits.
	assert.match(prePushHook, /trap .*EXIT/u);
	assert.doesNotMatch(prePushHook, /^\s*vp run check\s*$/mu);
});
...
void test("pnpm reuses only trusted native verification verdicts and still runs a frozen install", async () => {
	...
	assert.match(String(locate.get("run")), /\$\(pnpm cache path\)/u);
	assert.match(
		String(locate.get("run")),
		/require\("node:path"\)\.join\(process\.argv\[1\], "lockfile-verified\.jsonl"\)/u,
	);
	assert.equal(
		proof.getIn(["env", "PNPM_VERIFICATION_CACHE"]),
		`\${{ steps.pnpm-verification-path.outputs.path }}`,
	);
	assert.match(String(proof.get("run")), /statSync\(process\.argv\[1\]\)\.isFile\(\)/u);
	assert.match(String(restore.get("uses")), /^actions\/cache\/restore@/u);
	assert.match(String(save.get("uses")), /^actions\/cache\/save@/u);
...
void test("a change to the task graph or the hooks selects every quality leg", async () => {
	...
	for (const entry of [
		...
		assert.match(
			filter,
			new RegExp(`'${escapeRegExp(entry)}'`, "u"),
...
void test("Semgrep scans PRs, main and merge queues without a privileged trigger or mutable rules", async () => {
	...
	assert.match(
		...
		/^semgrep\/semgrep:[\d.]+@sha256:[a-f0-9]{64}$/u,
	);
	for (const [command] of source.matchAll(/docker run.*/gu)) {
		assert.match(command, /--network none/u, "the scanner runs without a network");
	}
	assert.match(source, /--test --config security\/semgrep/u);
	assert.match(source, /--disable-nosem\b/u);
	assert.match(source, /--strict\b/u);
	assert.match(source, /--error\b/u);
	assert.match(source, /--sarif-output\b/u);
	assert.doesNotMatch(source, /continue-on-error|secrets\.|--config (?:auto|p\/)/u);
...
void test("every Semgrep rule ships a positive and a negative fixture", async () => {
	...
	const fixtureSources = await Promise.all(
		files.filter((file) => !file.endsWith(".yaml")).map(async (file) => readFile(file, "utf8")),
	);
	const fixtures = fixtureSources.join("\n");
	for (const file of rulesets) {
		...
		for (const id of ids) {
			assert.match(fixtures, new RegExp(`ruleid: ${id}$`, "mu"), `${id} has no violating example`);
			assert.match(fixtures, new RegExp(`ok: ${id}$`, "mu"), `${id} has no compliant example`);
		}
	}
});
...
void test("toolchain cache producers cover Linux and Windows without repeating quality gates", async () => {
	...
	assert.match(String(checkout.get("uses")), /^actions\/checkout@/u);
...
void test("CI does not run CodeQL analysis or retain extraction-only compiler exceptions", async () => {
	...
	for (const file of await posixGlob(".github/workflows/*.yml")) {
		...
		assert.doesNotMatch(
			source,
			/github\/codeql-action\/(?:init|analyze)@|workflows\/codeql\.yml|needs\.CodeQL/u,
		);
	}
