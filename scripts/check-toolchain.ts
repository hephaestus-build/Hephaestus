...
const match =
	typeof packageManager === "string"
		? /^pnpm@(?<version>\d+\.\d+\.\d+)$/u.exec(packageManager)
		: null;
if (!match) {
...
if (typeof vitePlusPin !== "string" || !/^\d+\.\d+\.\d+$/u.test(vitePlusPin)) {
...
for (const hook of ["commit-msg", "pre-push"]) {
	...
	if (!indexed.startsWith("100755 ")) {
		throw new Error(`${file} must be committed with mode 100755`);
	}
}
...
const usesTsx = (command: string): boolean => /(?:^|[;&|])\s*tsx(?:\s|$)/u.test(command);
for (const [name, task] of Object.entries(tasks)) {
	for (const line of commandsOf(task)) {
		if (usesTsx(line)) {
			throw new Error(`vite.config.ts task ${name} must use Node directly`);
		}
	}
}
for (const file of [
...
if (!isRecord(overrides)) {
	throw new Error("pnpm-workspace.yaml must declare overrides");
}
...
for (const file of execFileSync("git", ["ls-files", ".github"], { encoding: "utf8" }).split("\n")) {
	...
	if (/^\s*java-version:/mu.test(source)) {
...
if (!Array.isArray(setupSteps)) {
	throw new TypeError("setup-toolchain must declare steps");
}
const setupStep = (
	...
	if (!step) {
		throw new Error("setup-toolchain is missing a step it must have");
	}
	return step;
};
...
const idOf = (step: Record<string, unknown>): string => {
	if (typeof step.id !== "string") {
		throw new TypeError("setup-toolchain step must carry an id");
	}
	return step.id;
};
...
if (
	!isDeepStrictEqual(String(withOf(pnpmCache).path).trim().split(/\s+/u).toSorted(), [
		"~/setup-pnpm/pnpm",
		"~/setup-pnpm/pnpm.exe",
	]) ||
...
const restoredPath = String(restored.run)
	...
	.flatMap(
		(line) =>
			/^\s*echo "(?<entry>\$PNPM_DEST[^"]*)" >> "\$GITHUB_PATH"$/u.exec(line)?.groups?.entry ?? [],
	);
if (
...
for (const [attempt, index] of pnpmSetupOrder.slice(1).entries()) {
	...
	if (backoff.if !== guard || !/^sleep \d+$/u.test(String(backoff.run))) {
...
if (
	!/^\s+pnpm install --offline --frozen-lockfile --ignore-scripts --filter webapp$/mu.test(
		dockerfile,
	)
) {
	throw new Error(
...
const pnpmImage = new RegExp(
	`ghcr\\.io/pnpm/pnpm:${version.replaceAll(".", String.raw`\.`)}@(?<digest>sha256:[a-f0-9]{64})`,
	"u",
);
const pnpmDigests = new Set(
	(
		...
		const digest = pnpmImage.exec(content)?.groups?.digest;
		if (digest === undefined) {
			throw new Error(`${file} must use the digest-pinned pnpm ${version} image`);
		}
		return digest;
	}),
);
if (pnpmDigests.size !== 1) {
	throw new Error("both Dockerfiles must pin one pnpm base digest");
}
// Those settings only bind if the image copies them and lets pnpm read them.
if (
	!/^COPY pi\/package\.json pi\/pnpm-lock\.yaml pi\/pnpm-workspace\.yaml \.\/$/mu.test(
		agentDockerfile,
	) ||
	!/^RUN pnpm install --prod --frozen-lockfile --ignore-scripts$/mu.test(agentDockerfile)
) {
	throw new Error(
...
if (
	!new RegExp(`^ARG NODE_VERSION=${nodeVersion.replaceAll(".", String.raw`\.`)}$`, "mu").test(
		dockerfile,
	) ||
	!new RegExp(
		[String.raw`^RUN pnpm runtime set node \$`, String.raw`\{NODE_VERSION\} -g$`].join(""),
		"mu",
	).test(dockerfile)
) {
...
const tracked = execFileSync("git", ["ls-files", "-z"], {
	...
	.split("\0")
	.filter(Boolean);
const forbiddenWord = new RegExp(`\\b${retired}\\b`, "iu");
...
const forbiddenCommand = new RegExp(
	`\\b${lane} (?:run|exec|changeset|--filter)\\b|"${lane}", \\[`,
	"u",
);
for (const file of tracked) {
	if (isHistorical(file) || !existsSync(file)) {
		continue;
	}
	if (forbiddenPath.test(file)) {
		throw new Error(`${file} is a retired tool artifact`);
	}
