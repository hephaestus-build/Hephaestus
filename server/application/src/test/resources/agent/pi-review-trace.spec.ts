...
void test("native Pi captures the final provider payload and a recoverable session without credentials", async () => {
	...
	const unsubscribe = session.subscribe((event) => capture.event(sessionId, event));
	try {
		await session.prompt("Inspect only the captured evidence.");
		capture.finish(0);
