...
public class GitHubAccessJobService {
    ...
    public GitHubAccessJobService(
            GitHubAccessPolicyService policies,
            @Qualifier("gitHubAccessSyncAdapter") IntegrationSyncRunner runner,
            SyncJobService jobs,
    ...
    public SyncJobDTO request(long workspaceId, long targetId, boolean preview) {
        long connectionId = policies.connectionForJob(workspaceId, targetId, preview);
    ...
    public void scheduled(long workspaceId, long targetId) {
        long connectionId = policies.scheduledConnection(workspaceId, targetId);
    ...
    private SyncJobDTO dispatch(SyncJobRequest request) {
        ...
        try {
            ...
            if (conflict.activeJob().getType() != request.type())
                throw new SyncStateConflictException(
                        "Wait for the current GitHub access job before starting a different pass",
        ...
        try {
            ...
            jobs.failStarted(started, "GitHub access dispatch rejected; retry when the executor is available");
            throw rejected;
        }
