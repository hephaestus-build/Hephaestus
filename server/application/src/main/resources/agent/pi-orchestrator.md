...
- `<contextRoot>/linked_work_items.json` — (PR) the same issues as records, for every issue the provider records the change as closing (`how: closes`) and every issue number the title, description, branch or commit messages mention (`how: mentions`), plus `unresolvedReferences[]`. How each is referenced — closing keyword, bare mention, branch — you read from `metadata.json`, `source_branch` and the commits; a mention alone does not establish guidance supplied or adopted by the author
