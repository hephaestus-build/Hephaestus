...
- `<contextRoot>/commits.json` — (PR only) the pull request's commits over the same range as `diff.patch`, oldest first in history order, with subject, body, timestamps, parent count and the number of files each non-merge commit touched. Read it before judging a commit-message or commit-scope practice. Per-commit diffs are not staged.
