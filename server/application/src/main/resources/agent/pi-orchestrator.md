...
- `<contextRoot>/review_threads.json`, `<contextRoot>/general_comments.json` — (PR) review threads with `id`, state, who resolved and `createdAt`, and every submitted decision oldest first with its `body` when one was written and `bot` when the provider classifies its author so; the non-inline conversation, its comments flagged the same way. Hephaestus's own notes are filtered out of all three
