...
criteria in `<practiceRoot>/<slug>.md` for the practice(s) scoped to this turn; `<practiceRoot>/all-criteria.md`
...
8. **Never fabricate context — confirm a file exists before you rely on it.** Before you base ANY observation on a context file
   ...
   `NO_REVIEW_OCCASION`. Forbidden: claiming "the repository contains
...
- `<contextRoot>/commits.json` — (PR only) the pull request's commits over the same merge-base-to-head range as `diff.patch`, oldest first in history order, each with `sha`, `subject`, `body` (present only when the message had one), `authored_at`, `committed_at`, `parent_count` (2 or more marks a merge commit) and, for a non-merge commit, `changed_files`, `additions` and `deletions`. Read it before judging a commit-message or commit-scope practice. `truncated:true` means the list was cut, so it is not the whole branch. Per-commit diffs are not staged.
