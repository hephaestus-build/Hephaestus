...
import { contextFile, inventoryIssues, isOpenState, readProjectInventory } from "../lib/context.ts";
...
export default async function linksTheChangeToItsIssue(
	...
	add(
		`${contextFile(contextReference, "metadata.json")} (title)`,
		issueNumberReferences(title),
		false,
	);
	add(contextFile(contextReference, "description.md"), closingReferences(body), true);
	add(
		contextFile(contextReference, "description.md"),
		issueNumberReferences(body).filter((n) => !closingReferences(body).includes(n)),
		false,
	);
	add(
		`${contextFile(contextReference, "metadata.json")} (source_branch)`,
		branchIssueReferences(branch),
		false,
	);
	...
	if (numbers.size > 0) {
		...
		directions.push(
			`No #number, closing keyword or branch number in the title, body or branch (${branch}); ${linked === null ? "linked_work_items.json was not captured" : `${String(linked.length)} linked work item(s) captured`}. Read ${contextFile(contextReference, "commits.json")} for a reference in history, then the inventory (${inventory === null ? "not captured" : `${String(metrics.inventoryIssues)} issues, ${String(metrics.inventoryOpenIssues)} open`}) for an open issue that plainly describes this work.`,
		);
	}
