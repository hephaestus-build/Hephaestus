...
function nearDuplicateIssues(
	...
	} else if (inventory?.truncated === true) {
		// Capped listing: the absence of an overlap among the LISTED subset does NOT prove uniqueness.
		directions.push(
			`Cross-artifact fact: project_inventory.json lists ${issues.length} issue(s) but is TRUNCATED (capped subset, not exhaustive); none LISTED has a strongly-overlapping title, but a duplicate may exist outside the listing — do not conclude this is unique from the inventory alone.`,
		);
	} else {
		directions.push(
			`Cross-artifact fact: ${issues.length} other issue(s) exist in project_inventory.json (full listing, not truncated); none has a strongly-overlapping title — absence of an overlap here reflects the exhaustive listing, not a capped subset.`,
		);
	}
...
export default async function issueStatesAnActionableProblem(
	...
	if (emptyOrTitleEcho && hasDeliverableType) {
		...
		directions.push(
			`Classification fact: empty body and no deliverable work-item type — looks like a board column / tracker rather than an authored work item.`,
		);
	...
	const nearDuplicateCount = nearDuplicateIssues(inventory, title, directions);
	const openPrTitleMatchCount = openPullRequestTitleMatches(inventory, title, directions);
