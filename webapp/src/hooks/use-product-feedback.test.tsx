...
describe("product feedback wire contract", () => {
	it("sends account feedback without a workspace or implicit page data", async () => {
		...
		await act(async () => {
			await expect(
				result.current.submit({ kind: "BUG", message: "Cannot open reviews" }),
			).resolves.toBe(true);
		});
	...
	it("keeps a failed submission actionable and does not automatically retry a POST", async () => {
		...
		await act(async () => {
			await expect(result.current.submit({ kind: "FEEDBACK", message: "A draft" })).resolves.toBe(
				false,
			);
		});
	...
	it("removes a submitted global survey from every cached workspace even when refresh fails", async () => {
		...
		await act(async () => {
			await expect(
				result.current.submit(surveyInvitation, [{ questionId: "useful", rating: 4 }]),
			).resolves.toBe(true);
		});
	...
	it("drops a survey that was already settled elsewhere from the menu", async () => {
		...
		await act(async () => {
			await expect(result.current.submit(surveyInvitation, [])).resolves.toBe(false);
		});
	...
	it("replaces an earlier response conflict with the latest decline failure", async () => {
		...
		await waitFor(() => expect(result.current.error).toContain("already answered or declined"));
		await act(async () => {
			await expect(result.current.decline(surveyInvitation.id)).resolves.toBe(false);
		});
	...
	it("thanks a respondent for the study a research survey belongs to, and a sender by kind", async () => {
		...
		await screen.findByText("Thank you — your answers are on their way to the Hephaestus team.");
		...
		await screen.findByText("Thanks — your idea is on its way to the Hephaestus team.");
	});
	...
	it("undoes a decline from the toast", async () => {
		...
		await waitFor(() => expect(result.current.query.data).toHaveLength(1));
		await act(async () => {
			await expect(result.current.decline(surveyInvitation.id)).resolves.toBe(true);
		});
	...
	it("sends one request when feedback is submitted twice at once", async () => {
		...
		await act(async () => {
			...
			await expect(result.current.submit({ kind: "FEEDBACK", message: "Duplicate" })).resolves.toBe(
				false,
			);
