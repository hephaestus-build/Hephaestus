...
export const Default: Story = {
	play: async ({ canvas }) => {
		await expect(canvas.getByRole("heading", { name: "No content found" })).toBeVisible();
	},
};
...
export const LongDescription: Story = {
	...
	play: async ({ canvas }) => {
		...
		const panel = action.closest('[data-slot="empty"]');
		if (!panel) {
			throw new Error("Empty state panel is missing");
		}
