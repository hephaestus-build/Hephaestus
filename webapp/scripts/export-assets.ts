...
await mkdir(path.resolve(publicDirectory, "brand"), { recursive: true });
...
for (const [source, targets] of [
	...
	for (const target of targets) {
		await copyFile(path.resolve(sourceDirectory, source), target);
	}
}
...
try {
	...
	const captureMark = async (
		...
		await page.screenshot({ path: target, omitBackground: !opaque });
	};
	...
	await copyFile(socialCard, path.resolve(publicDirectory, "hephaestus-social-card.png"));
...
async function captureReadmeImage(
	...
	await surface.screenshot({
		path: path.resolve(readmeImageDirectory, `${capture.name}-${theme}.png`),
	});
}
