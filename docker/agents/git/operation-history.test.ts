...
void test("historical reads use witnessed roots and Git file bytes, not injected refs", async () => {
	...
	try {
		...
		await symlink("source.txt", join(directory, "link"));
		git("add", ".");
		git("commit", "-m", "Main");
		const head = git("rev-parse", "HEAD");
		git("update-ref", "refs/remotes/origin/feature", feature);
		git("tag", "-a", "-m", "blob tag", "blob-tag", git("rev-parse", "HEAD:source.txt"));
		await writeFile(
			join(directory, ".git/hephaestus-captured-refs"),
			`commit ${feature} refs/remotes/origin/feature\nblob ${git("rev-parse", "blob-tag^{}")} refs/tags/blob-tag\n`,
		);
		...
		assert.equal(
			execFileSync("tar", ["-xOf", "-", "0"], { input: batch.stdout, encoding: "utf8" }),
			"main\n",
		);
		assert.equal(
