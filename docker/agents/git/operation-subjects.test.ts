...
void test("subject scanning reaches references beyond five hundred commits without file details", async () => {
	...
	try {
		...
		const result = spawnSync(
			...
			[fileURLToPath(new URL("./operation.ts", import.meta.url))],
			{
				input: JSON.stringify({
					...
					revisions: [git("rev-list", "--max-parents=0", "HEAD"), git("rev-parse", "HEAD")],
				}),
				encoding: "utf8",
				timeout: 10_000,
		...
		assert.ok(
			commits.stdout.indexOf('"subject": "Reference #1"') <
				commits.stdout.indexOf('"subject": "Reference #600"'),
		);
	} finally {
