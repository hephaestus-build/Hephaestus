...
| **External actor** | Read-only mirror of a git-provider account (USER / ORGANIZATION / BOT). Authors activity (PRs, issues, reviews). **Not a principal** — does not log in, does not have preferences, does not impersonate. | `integration.scm.domain.user.User`; SQL table `"user"` | `(provider_id, native_id)` |
| **Account vs external actor** | Two distinct entities. `AccountFeature`, `IdentityLink`, `issued_jwt` reference `Account`. `WorkspaceMembership`, `Issue.author_id`, `PullRequest.merged_by`, review comments and team memberships reference the external actor — moving membership onto `Account` is [ADR 0019](decisions/0019-workspace-membership-keyed-on-account.md), still proposed. A single login flow may create both: one `Account` (the principal that will hold the cookie) plus one `IdentityLink` whose external-actor reference points to the matching mirror row already synced from the IdP. | — | — |
| **`WorkspaceMembership`** | External actor ↔ `Workspace` ↔ `WorkspaceRole`. `user_id` points at the SQL table `"user"`, the external actor, not at `Account` ([ADR 0019](decisions/0019-workspace-membership-keyed-on-account.md) would change that). | `workspace.WorkspaceMembership` | composite `(workspace_id, user_id)` |
