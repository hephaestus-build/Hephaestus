...
| **`WorkspaceMembership`** | External actor ↔ `Workspace` ↔ `WorkspaceRole`. `user_id` points at the SQL table `"user"`, the external actor, not at `Account`; re-keying it on `Account` is [ADR 0019](decisions/0019-workspace-membership-keyed-on-account.md). | `workspace.WorkspaceMembership` | composite `(workspace_id, user_id)` |
