...
the rule JGit's `RawText.isBinary` applies to the diff the same review reads, so the tree and the diff
agree on what is text; it is neither staged nor counted against the bound, and the capture records the
exclusion as `BINARY_FILE_EXCLUDED`. The rule is a heuristic over a blob's opening bytes: a binary
format that opens as text is staged, and a UTF-16 text file is not.
