...
What the tree stages is source text. A blob whose opening bytes contain a NUL is not staged, and the
capture says so — nothing a review can read or quote comes out of an image, an archive or a compiled
artifact, and the size bound is what the surrounding code is bought with. One repository's
documentation screenshots spent the whole bound before the walk reached a single source file.
