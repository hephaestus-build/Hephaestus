...
server answers every repository citation of a review from one native Git read of the snapshot it
pins by the `inputs/sources/scm/repo/.git/HEAD` and captured-refs digests: a path that does not exist at that revision, or a
