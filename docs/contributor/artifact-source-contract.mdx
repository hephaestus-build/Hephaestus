...
The machine-readable [catalog](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/resources/contracts/artifact-source/1.0.0/catalog.json),
...
The tree stages text. The review's tools read and grep; an image, an archive or a compiled artifact
shows them nothing and would spend the bound the surrounding source is bought with. A blob is binary by
the rule JGit applies when it renders the diff the same review reads — a NUL byte, or a carriage return
not followed by a line feed, in its opening 8 KiB — so the tree and the diff agree on what is text. A
binary blob is neither staged nor counted against the bound, and the capture records the exclusion as
`BINARY_FILE_EXCLUDED`. The rule is a heuristic: a binary format that opens with 8 KiB of text is
staged, and a UTF-16 text file is not.
