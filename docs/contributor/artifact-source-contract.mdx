...
What the tree stages is meant to be source text. A blob whose opening bytes contain a NUL — git's own
test for binary content — is neither staged nor counted against the bound, and the capture records the
exclusion. The bound is what the surrounding code is bought with, and an image, an archive or a
compiled artifact spends it without adding a line anyone can quote; one repository's documentation
screenshots spent the whole bound before the walk reached a single source file. The test is a
heuristic, so a binary format with no NUL in its opening bytes is still staged.
