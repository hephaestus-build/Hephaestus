...
`scm.repository.tree` to include contextual access to reachable Git history under the source's governance
and retention policies. The reviewed commit and tree identify the checkout; captured artifact digests
additionally pin the exported refs and Git database, which can change between captures of the same commit.
...
Git history can guide exploration, but a quote from `git show <commit>:<path>` is not verified merely
because the corresponding object is present in a captured pack file. Citations must name a captured
artifact containing the quoted text, such as the reviewed-commit file or pull-request diff; shell output
does not become a new evidence artifact. Historical quotations without such an artifact cannot ground
delivered feedback.
