...
Hephaestus may review work against a practice only from evidence with explicit semantics. Contract `1.1.0` defines the
...
The machine-readable [catalog](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/resources/contracts/artifact-source/1.1.0/catalog.json),
...
- Three `1.1.0` sources cannot be logically empty — the pull-request, issue, and document core records.
...
dependency, or normalization requires a new contract version. Contract `1.1.0` expands
