...
source in the pinned manifest, and the quote occurs in the digest-addressed artifact. Observations already persisted
by the `secret-diff-scanner` detector are the one exception delivery still accepts: their citations carry a quote
digest and a redacted location instead of the quote, and only the location is kept; no current detector produces
them. Delivery fails atomically on any mismatch.
...
A repository citation names `scm.repository.tree`, the captured `inputs/sources/scm/repo/.git/HEAD` witness as its
`artifactPath`, a repository-relative `path`, the exact quote and line range, and optionally a
`revision` — a full commit id from the captured history; omitted, it means the reviewed commit. The
server answers every repository citation of a review by opening, with JGit, the snapshot it pins by the
...
`scm.repository.tree` citations may select a revision.
