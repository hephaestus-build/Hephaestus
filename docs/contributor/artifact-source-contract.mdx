...
Hephaestus may review work against a practice only from evidence with explicit semantics. Contract `1.2.0` defines the
...
> A source failure is never `NOT_APPLICABLE`, a strength, or evidence that desired behaviour is absent.
...
## Source catalog (`1.2.0`)

The machine-readable [catalog](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/resources/contracts/artifact-source/1.2.0/catalog.json),
its adjacent JSON Schemas, and the checks in
[`gate:contracts`](https://github.com/hephaestus-build/Hephaestus/blob/main/scripts/validate-artifact-source-contracts.ts)
...
- Three `1.2.0` sources cannot be logically empty — the pull-request, issue, and document core records.
...
- Two cannot be partial: the pull-request diff and the document core record. An available diff is
  additionally pinned. Its `requiredQuality` is `COMPLETE`: a verified empty range is available evidence.
  Each practice still needs its own occasion and supported observation; capture readiness assigns neither
  applicability nor an outcome. Missing, failed or incomplete required captures remain unready.
...
dependency, or normalization requires a new contract version. Contract `1.2.0` expands
`scm.repository.tree` from a `.git`-free tree to a checkout with the Git history reachable from the
reviewed commit and the mirrored branches and tags, under the source's governance and retention
policies. The reviewed commit and tree identify the checkout; the manifest additionally digests the
`inputs/sources/scm/repo/.git/HEAD` and captured-refs witnesses, which pin the exported refs — those can change between two
captures of the same commit.
...
Every bundled practice that requires the pull-request diff lists `scm.repository.tree` as `CONTEXTUAL`.
The tree supplies surrounding code that the diff may not show. This binding does not remove the
repository requirement for capturing the pinned diff: if that required capture fails, the practice
cannot proceed. A missing local mirror can be cloned during review preparation.
...
The workspace gate decides whether a review is admitted at all, before any source is captured. It refuses
the signal with a typed reason rather than silently doing nothing, and the reason — not the caller —
decides whether the occurrence is terminal or re-offered later.

Two of its checks bear on evidence, and only these two; the rest (is the workspace active, is practice
review switched on, is a model bound, is a budget left, has this artifact been reviewed too recently) are
in [When a workspace goes quiet](/admin/practice-review#when-a-workspace-goes-quiet).

- **Review coverage**, checked first. The repository/base-branch axis and the person axis are intersected,
  and both must admit the work. A repository or base branch outside the selection, a selected list left
  empty, or an author who is not a member of the workspace is refused with terminal `OUT_OF_REVIEW_SCOPE`.
  Work with no identifiable author is refused with retryable `SUBJECT_UNLINKED` instead, because an
  identity can still arrive later.
- **Practice autonomy.** A practice at `OFF` admits no review; `HUMAN_APPROVAL` and `AUTOMATIC` both run it
  and differ only in whether a person must authorize release onto the work. When every practice bound to a signal sits at
  `OFF`, the signal is refused with retryable `PRACTICE_AUTONOMY_OFF`, so turning a practice back on lets
  the waiting occurrence through.

Both are defined in the [practice review glossary](./practice-review-glossary.mdx). Neither is an evidence
concern — they decide whether evidence is collected at all.
...
runtime makes no claim that the change is focused, sprawling, or `NOT_APPLICABLE`. No stance can demand
...
source in the pinned manifest, and the quote occurs in the digest-addressed artifact. Delivery fails atomically on
any mismatch.
...
A quote of code is verified against the captured repository, whichever of its two sources it names. A
repository citation names `scm.repository.tree`, the captured `inputs/sources/scm/repo/.git/HEAD` witness as its
`artifactPath`, a repository-relative `path`, the exact quote and line range, and optionally a
`revision` — a full commit id from the captured history; omitted, it means the reviewed commit. A change
citation names `scm.pull-request.diff`, the pinned `inputs/context/change.json` as its `artifactPath`, the
`OLD` or `NEW` side, and the same path, quote and lines; its revision is the side's commit of the pinned
range, and a path the change does not touch withholds the observation. The server never stages a diff:
the container derives `work/change/diff.patch` from the checkout with `git`, and the line annotations it
carries are the coordinates a change citation names. The
server answers every code citation of a review by opening, with JGit, the snapshot it pins by the
`inputs/sources/scm/repo/.git/HEAD` and captured-refs digests: an object the snapshot never captured, a path that does not exist at that revision, or a
quote that does not match those lines, withholds that observation and records
`QUOTE_LOCATION_MISMATCH` against the citation. Shell output is not an artifact — a
quote is verified against the Git object, never against what a command printed — and only
`scm.repository.tree` citations may select a revision, only `scm.pull-request.diff` citations a side.
...
- `gate:contracts` validates every contract resource and the bundled practice catalog against their schemas,
