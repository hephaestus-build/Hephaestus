...
records what it follows through `replaces_id`. The Practice profile keeps the ledger and not the page: a
new card about a practice supersedes the card still **open** about it, read or not, because the page is a
list of habits to work on and one habit gets one open card; a card that is closed — resolved by the work
or by the developer, or closed because the practice changed — is never superseded and stays as the record
beside the new one (`FeedbackSupersession#replaceOpen`, decided from `PreviousInAppFeedback`). Issue
...
| **Marked as addressed**  | The developer recorded `ADDRESSED` or `NOT_APPLICABLE` as their response to the feedback (`InAppFeedbackEvidence#resolvedByDeveloperAt` over `FeedbackResolution#resolves`; a dispute leaves it open). The other way a piece of feedback resolves; the earlier of the two is what the card and the summary report, and the card carries the moment as `resolvedByDeveloperAt` so no reader decides it again. | dismissed, acknowledged, done                       |

The count is a piece of reviewed work, bundled as the trend bundles it — one opportunity per piece of
work, read off its latest run — so a re-review that changes what the latest run said changes the answer
with it. A run of three that completed once stays a resolution: a later slip is new feedback's business.
...
after the card was prepared: the card stays on the page with `practiceChangedAt` set and says so, is not
...
- **Precedence** when the page names one event per practice: resolved feedback, then new feedback, then
  a standing that slipped, then one that moved up, then a first observation, then a trend that turned,
  then a practice that held.
- **A held practice is dropped** from what held when the same practice slipped or got new feedback in
  the window: holding and moving up are not a contradiction, holding and slipping are.

## Machine names

The `artifact` → `reviewedWork` rename is scoped to the HTTP DTOs, so the webapp's `artifactKind`
and `artifactId` search params and its `$artifactKind` route segment keep their names on purpose.
...
| Resolved by the work         | `cleanWork`, `cleanNeeded`, `resolvedByWorkAt` | `WorkResolution`            | derived, never stored                                                         |
| Marked as addressed          | `resolution: ADDRESSED`, `resolvedByDeveloperAt` | `Reaction.resolution`        | `reaction.action`                                                             |
