...
| **Job folder**                      | The directory the owning worker renders for one review attempt: permitted context areas as files plus a working copy of each permitted repository. [ADR 0041](https://github.com/hephaestus-build/Hephaestus/blob/main/docs/decisions/0041-compose-1x-kubernetes-2.md#evidence-admission-and-deletion) owns its lifetime.                                                 | workspace mount, evidence store, content-addressed store          |
...
| **When evidence is insufficient**   | The required behavior when evidence requirements are not met. Skip automated review when the required source conditions are not met.                                                                                                                              | fallback, error handling                                          |
...
| **Practice autonomy**               | Who may authorize feedback pushed onto the work for one practice: nobody (`OFF`), a workspace owner or administrator (`HUMAN_APPROVAL`), or the system (`AUTOMATIC`). The pull lanes — the practice page and the mentor — deliver at every autonomy but `OFF`. Nullable at practice and group levels, where null means “inherit”. | review tier, loudness tier, shadow mode, enabled                  |
...
`COALESCED` means an issue-update snapshot was displaced before admission. It is suppressed, not pending:
the reaper does not retry it, but a later live transition back to that content can offer it again.
It says nothing about whether earlier observations were resolved. See
[Issue lifecycle occasions](./practice-review-pipeline.mdx#issue-lifecycle-occasions) for revision identity.

## Practice autonomy

`PracticeAutonomy` answers one question: **who may authorize feedback pushed onto the work for this
practice?** It is not the whole delivery policy; recipient preferences, workspace controls, artifact
validity, suppression and the final egress guard still apply.

| Autonomy         | Review runs | Feedback is composed | Release authority on the work          | Practice page and mentor |
| ---------------- | ----------- | -------------------- | -------------------------------------- | ------------------------ |
| `OFF`            | no          | no                   | nobody                                 | nothing                  |
| `HUMAN_APPROVAL` | yes         | yes                  | a workspace owner or administrator     | delivered                |
| `AUTOMATIC`      | yes         | yes                  | the system, subject to delivery policy | delivered                |

At `HUMAN_APPROVAL`, feedback **pushed** onto the work needs approval before release. Feedback on a
practice page or in a mentor turn is **pulled** by the developer and does not need comment approval.
It remains subject to channel access controls, the origin rule and delivery rules such as attribution,
corroboration and cooldown. The rule has one home, `PracticeAutonomy.delivers(channel)` over
`FeedbackChannel.pushed()`.
...
system may release feedback onto the work without a person.
...
The review subject for pull requests, merge requests, and issues is their author, and a reviewer run
is about the reviewer it names. A practice may declare another relation as its `subject`: `REVIEWER`
for the review practices, `MERGER` for a practice about the act of merging. A review is filed against one
person, so a `MERGER` practice is reviewed in an author run only when the author is the one who merged
(`merged_by` is staged in `metadata.json`) and is withheld otherwise — a lapse at the merge is never filed
against someone who did not merge. `ReviewSubjectStatus` records which of four things the subject turned
out to be:
...
- **At `COMPOSITION` the contributing practices are not yet known**, so `PRACTICE_AUTHORITY` is
  `NOT_APPLICABLE`. A document or conversation still has a review subject: repository-less coverage,
  recipient consent and target eligibility are checked when that subject is resolved. External surfaces
  also apply Silent Mode and the delivery pause; `IN_APP` does not, because it never leaves Hephaestus.

The artifact lane is evaluated again at `AUTOMATIC`, at `APPROVED` when a person released it, and at `EGRESS`
immediately before the provider call. The `IN_APP` and `CONVERSATION` surfaces are checked at `COMPOSITION`;
each lane's router also applies its origin-and-autonomy predicate and per-lane rules, recording routing
refusals separately. Prepared conversation feedback must pass an `EGRESS` policy evaluation before it
enters mentor context; source visibility and Slack consent gates can exclude it before that evaluation.
...
| **Supersession**           | Replacing feedback with newer feedback on the same thread: prepared, still-unread feedback on the conversation lane, and the still-open card on the Practice profile.                                                                                | edit, update, delete                                       |
...
On the conversation lane supersession only ever claims `PREPARED` feedback. Say **superseded** for
feedback that was prepared but replaced before anybody read it, and **continued** for newer feedback
written beside feedback that was already read: nothing received is ever un-said, and the newer row still
records what it follows through `replaces_id`. On the Practice profile a new card about a practice
replaces the card still **open** about it, read or not, so the page holds one open card per practice; a
card that is closed — resolved by the work or by the developer, or closed because the practice changed —
is never superseded and stays as the record beside the new one (`FeedbackSupersession#replaceOpen`,
decided from `PreviousInAppFeedback`). Issue
reviews may edit the prior issue comment in place, so their ledger entry is retired when that visible text
is replaced. Pull request reviews do not: every new review posts a new summary, while a retry of the same
review reconciles its existing comment by marker.

### Recorded review history

The history context contains each recorded observation and the feedback already delivered or prepared.
A location groups observations about the same piece of work; it does not establish semantic equivalence
between their behaviors. Comparing outcomes at a shared location or omitting a prior observation does
not establish that a concern was resolved or regressed. Responses apply to the specific observations
bound to the feedback the developer received.

### How feedback resolves

A piece of [feedback](./practice-feedback-language.md) on the Practice profile resolves in one of two
ways, and the two are held apart everywhere the resolution is shown.

| Term                     | Plain meaning                                                                                                                                                                                                                                                                                                                  | Do not use for this concept                         |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------- |
| **Resolved by the work** | Three pieces of the developer's reviewed work in a row, reviewed after the feedback was prepared, came back clean on the practice. Computed from the observations when the feedback is read and never stored; a problem starts the count over, and a piece of work reviewed several times counts once, at its newest review. | auto-resolved, closed by the system, streak, passed |
| **Marked as addressed**  | The developer answered the card with *Addressed* or *Not applicable*; pressing the answer again takes it back and reopens the card. A dispute leaves it open. Whichever way resolved the feedback first is the one the card and the summary report.                                                                        | dismissed, acknowledged, done                       |
| **Fell back**            | Open feedback the developer's work had started to answer, with at least one clean piece of work toward the three, until a piece of work reviewed in the overview's window raised the problem again and the count went back to none. Closed feedback never falls back.                                                        | reset, regressed, reopened                          |

A run of three that completed once stays a resolution: a later slip is new feedback's business.
The profile summary reports a `FEEDBACK_RESOLVED` change with who resolved it, `WORK` or `DEVELOPER`,
and never merges the two into one count.

A card is **closed** when it is resolved either way, and also when its practice changed its review rules
after the card was prepared: the card stays on the page with `closedBy: PRACTICE_CHANGED` and says so, is not
resolved by anything, and its evidence is kept even though it is no longer current, whereas evidence whose
source may no longer be cited still hides the card. A closed card leaves the Practice profile 30 days after
it closed, applied when the page is read and never by changing the row
(`InAppFeedbackService.CLOSED_CARD_STAYS`); the ledger keeps it.

### Practice profile overview

The overview is the Practice profile's account of what held and what changed, read off the standings
and the feedback ledger and composed into sentences on the page; nothing in it is a second standing or
resolution rule. Its rules, in one place:

- **The window** every change is measured over opens at the review run before the latest one and
  closes now. With fewer than two runs it spans the standing's own look-back of 90 days, and a previous
  run older than that is clamped to it, since no standing reaches further back.
- **Only feedback prepared inside the look-back** is reported. Older feedback is left out even when
  the developer answers it inside the window, because the work that may have closed it first lies
  before anything the overview reads.
- **One change per practice and type, the newest.** A practice that got two pieces of feedback in the
  window is reported once for each type of change, by the newer piece.
- **Precedence** when the page names one event per practice: resolved feedback, then feedback that
  fell back, then new feedback, then a standing that slipped, then one that moved up, then a first
  observation, then a trend that turned, then a practice that held.
- **A held practice is dropped** from what held when the same practice slipped, fell back or got new
  feedback in the window: holding and moving up are not a contradiction, holding and slipping are.
...
| Reviewed work                | `reviewedWork` in the HTTP DTOs; `artifactKind`, `artifactId` in webapp search params and routes | `ReviewedWorkRefDTO` | `artifact_kind`, `artifact_id`                                              |
...
| Materialization              | `artifacts`                        | `SourceArtifact`                           | manifest and the attempt's staged input files                                 |
...
| Resolved by the work         | `cleanWork`, `cleanNeeded`, `closedBy: WORK` | `WorkResolution`            | derived, never stored                                                         |
| Marked as addressed          | `resolution: ADDRESSED`, `closedBy: DEVELOPER` | `Reaction.resolution`        | `reaction.action`                                                             |
| Closed feedback              | `closedAt`, `closedBy`             | `FeedbackClosure`                          | derived, never stored                                                         |
| Who resolved feedback        | `resolvedBy`                       | `ProfileChangeDTO.ResolvedBy`              | derived, never stored                                                         |
| Fell back                    | `FEEDBACK_RESET`                   | `ProfileChangeDTO.Type.FEEDBACK_RESET`     | derived, never stored                                                         |
| Standing move direction      | `direction: UP`, `DOWN` on a standing or group move; absent for a move into or out of a silence | `ProfileChangeDTO.Direction`, from `PracticeStandingDTO.Standing.rank()` | derived, never stored |
| Holds as                     | `holdsAs`                          | `BundledPracticeCatalog`                   | bundled catalog, never stored — defined in the [feedback language](./practice-feedback-language.md) |
...
The [product vocabulary](./practice-feedback-language.md#observation-assessment-axes) owns observation
status and result labels. The same axes are used by the generation tool, persisted observation and
read DTOs; the [feedback schema](./practice-feedback-schema.md#evidence-warrants) owns their warrants.

A readiness refusal belongs to the capture stage and creates no observation. It is not an assessment
status. NOT_APPLICABLE and UNDETERMINED describe distinct facts established by reading available
evidence, and neither contributes to the assessed-result denominator.
...
8. Never write bare **binding**. Two unrelated things carry the word — a *practice binding* (an occasion
   ...
   purpose, `WorkspaceAgentBinding`). Use `REVIEW_MODEL_UNBOUND` for a missing AI model.

## Member AI choice

The [product vocabulary](./practice-feedback-language#member-onboarding-and-ai-choices) defines member
onboarding, data handling, its tiers, and the AI choice as a ceiling. An admin declares one fact
per model, who operates it, and Hephaestus derives the tier from it: **In-house**, **Cloud** or
**Not declared**. A developer answers **In-house**, **Cloud** or **No AI**. The choice belongs to the
account and holds in every workspace. A practice review uses the loosest model binding of the
reviewing workspace within the ceiling of the developer it is about. Submission, queued execution,
model requests, and feedback delivery recheck that choice. A developer no account resolves to has
not answered; only a workspace that requires the choice keeps AI off their work.
A missing required choice or No AI is an intentional refusal, never missing evidence. Suppressed
occurrences do not automatically replay when the developer later enables AI.
