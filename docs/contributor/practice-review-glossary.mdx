...
Approval exists for what is **pushed**: a note on the work is public to everyone who reads the pull request
and reversible only by deleting it, so a person releases it. The practice page and a mentor turn are
**pulled** — the developer reads them on request and nobody else sees them — so every practice that admits
review at all delivers there, subject to the origin rule and the lane's own rules (author-attributed,
corroborated, cooldown). The rule has one home, `PracticeAutonomy.delivers(channel)` over
