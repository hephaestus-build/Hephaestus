...
| **Marked as addressed**  | The developer recorded `ADDRESSED` or `NOT_APPLICABLE` as their response to the feedback (`FeedbackResolution#resolves`; a dispute leaves it open). The other way a piece of feedback resolves; the earlier of the two is what the card and the summary report.                                                              | dismissed, acknowledged, done                       |
...
| Marked as addressed          | `resolution: ADDRESSED`            | `Reaction.resolution`                      | `reaction.resolution`                                                         |
...
A warrant is written under one name and read under another: `report_observation` takes
`evidence.exhaustiveSearch`, `evidence.exclusion` and `evidence.missingEvidence`, and the observation
stores and serves them as `evidence.search` (`lookedFor`, `consulted`, `boundary`),
`evidence.inapplicability` (`subject`, `consulted`, `ruledOutBy`) and `evidence.undecidability`
(`openQuestion`, `wouldSettleIt`) — one per observation, decided by its outcome, beside the citations
every observation owes.
