...
sequenceDiagram
    ...
    PR->>PR: vp exec changeset (bump + operator-facing description)
...
and generates a Syft inventory plus SPDX and CycloneDX SBOMs. One native Trivy `vuln,license` scan
supplies both the vulnerability and license report paths, avoiding duplicate image analysis while
keeping both evidence consumers bound to the same subject. `scripts/verify-release-evidence.ts` checks that every document is bound to that
...
- **Don't mention automatic migrations** — the release notes get an automatic "back up before
  ...
  changeset stays user-facing. The same fact is written into the notes as an invisible
  `<!-- hephaestus:schema-migrations=… -->` comment, which a running instance's update check reads to
  flag the newer release in the admin overview. Only a migration that requires operator action belongs in the summary
...
Publishing a release does not deploy it. Hosts follow a signed channel and apply it themselves, so
moving an environment is a separate, deliberate step: run the **Promote** workflow for that
environment and release, which the Production environment's required reviewer gates. The host picks
it up on its next reconcile — [Pull-Based Deployment](../admin/pull-based-deployment) has the
mechanics, and the environments are listed in [CI/CD](./ci-cd.mdx#environments). Verify staging
before promoting production. Hotfixes have no separate process: PR → changeset → merge → merge the
Version PR.

## Version management

Version numbers in source manifests stay at their placeholders (`webapp/package.json` and `server/build.gradle.kts`
