...
| Build → App Server: Database | Surefire `database` group against PostgreSQL; the PostgreSQL 18 backup-restore drill | `vp run test:postgres-restore` |
...
A default-branch push reuses the packaged reactor when a successful release-candidate merge group
