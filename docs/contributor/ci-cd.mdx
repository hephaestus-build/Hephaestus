...
| Quality → Tooling and Docs | `scripts/**`, `docker/agents/**`, `load-tests/**`, `docs/**`, tsconfigs, oxlint and oxfmt configs, the task graph (`vite.config.ts`), `.changeset/**`, `commitlint.config.ts`, instruction files, `renovate.json`, `openapitools.json` |
