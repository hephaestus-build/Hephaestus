...
| Quality → Webapp | `vp check`, presentational components, story prose, docs colour tokens, unit tests, Vite build with a `routeTree.gen.ts` diff | `vp run check:webapp`, `test:webapp`, `build:webapp` |
...
The image-reference, `security.txt` and Liquibase guards in `ci-security-scan.yml` stay independent. Inline `nosemgrep` suppression is disabled, so an
...
CodeQL analysis is disabled: CI has no extraction, query-analysis or scheduled CodeQL job, and
GitHub's CodeQL default setup must remain disabled. This removes CodeQL's interprocedural security
analysis; the remaining checks are not an equivalent replacement. GitHub Code Quality is a separate
maintainability service and remains enabled.

The pinned `github/codeql-action/upload-sarif` steps publish results from other scanners; they do
not run CodeQL analysis. Keep those uploads so Semgrep, Trivy and Scorecard retain their reporting.
Normal Java builds retain NullAway and fatal compiler warnings without an extraction-only bypass.
...
cover the GraphQL and OpenAPI build plugins. `server/build.gradle.kts` forces their vulnerable
transitive dependencies to patched versions; check `./server/gradlew -p server buildEnvironment`
...
| is behind `main` on schema | Merge `main` in; the next push previews automatically |
...
`Build`; release-version pushes repeat those checks. Main still packages and publishes images.
Same-repository pull requests also run `Build` and `Docker` regardless of paths, because
...
- A run on `main` that leaves a per-commit record — `semgrep.yml`, `scorecard.yml`,
...
```bash
# Webapp
vp run check:webapp          # vp check: format and lint; the lint is type-aware and type-checks
