...
valence, an assessment. Admission removes assessment from an `INCONCLUSIVE` observation, so a measurement that
hedges cannot attach an unearned strength to the series. For a practice targeting harmful behaviour,
`PRESENT/GOOD` is inconsistent and admission records an `incoherent_assessment` refusal. It does not rewrite
the observation as `NOT_APPLICABLE`: an inconsistent assessment establishes nothing about applicability.
The callback answers a refusal with HTTP 422, not a retryable server failure.
