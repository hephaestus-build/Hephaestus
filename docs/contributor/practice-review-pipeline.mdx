...
settles on after ten quiet minutes (at most an hour after the first push), marking the heads it overtook
`SUPPRESSED / COALESCED`. The workspace cooldown delays that settlement instead of refusing it, so the newest
head is never the one dropped. A push overtaken by a merge or a close is left to that occasion.
...
Capture projects; it does not interpret. A source reaches the sandbox as the stored record shaped into
JSON, or as a checkout — never as a summary, a count, an excerpt or a diff computed on the server. The
change a pull request review is about is captured as its pinned `base_sha` and `head_sha`
(`inputs/context/change.json`) beside the checkout, and the container derives the patch, its statistics,
the changed files and the commits from that checkout with `git` before the model runs
(`pi-change.ts`, under `work/change/`). Practice-specific extraction lives in the practices' precompute
scripts, which run in the same container on the same files. The server reads the change only to
decide which practices have their declared subject in it (readiness) and to verify what comes back
(admission) — see the [workspace ABI](./agent/workspace-abi.mdx) for the layout.
...
and it is the only prose the observation carries anywhere. The measurement emits no next step: there is no
...
`NOT_APPLICABLE` observation can record that no occasion exists; missing required sources remain
