...
The secret scan runs here, at input preparation, over the diff a pull-request review captures, and its
hits are stored in the same `evidence_snapshot`, bound to the staged diff by digest. Admission turns those
recorded hits into redacted observations without starting a container inside the request; a snapshot
whose verdicts belong to some other capture is inadmissible.
...
For a practice targeting harmful behaviour, PRESENT/GOOD is inconsistent and admission records an
`incoherent_assessment` refusal. It does not rewrite this to NOT_APPLICABLE: inconsistency proves
nothing about applicability. The callback answers a refusal with HTTP 422, not a retryable failure.
...
Admission is single-flight per attempt. Verifying a historical citation may start a Git container, so a
runner that repeats a submission whose answer did not arrive joins the admission already running and
receives its answer; a repeat carrying a different payload is refused as a conflict, and an attempt that
was already admitted answers with the recorded result.
