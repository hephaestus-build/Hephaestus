...
- **Repository or Slack channel sync error?** Open **Administration → Integrations** and check the affected resource's last error. Check its access in the connected service, then run a new sync. The error clears after that resource syncs successfully; a skipped resource keeps its last error.
