...
[`docker/self-host/`](https://github.com/hephaestus-build/Hephaestus/tree/main/docker/self-host)
...
- These are deployment limits, not a claim that every workload fits. Before raising webhook volume,
  ...
  [Compose capacity qualification](https://github.com/hephaestus-build/Hephaestus/tree/main/load-tests) on
...
- Container defaults are 4 CPUs / 512 PIDs for the application server and 2 CPUs / 256 PIDs for
  the webhook server. Override them with the corresponding `APPLICATION_SERVER_*` or
  `WEBHOOK_SERVER_*` limit variables. Keep PID limits on roles with Docker access.
...
- The [Node.js](https://nodejs.org/en/download) version pinned in `package.json#devEngines.runtime`
  and [Cosign](https://docs.sigstore.dev/cosign/system_config/installation/) for release-lock
  verification.

## 1. Get the files

Check out the newest version from the
[releases page](https://github.com/hephaestus-build/Hephaestus/releases):

```bash
VERSION=0.77.4   # the release you are installing, without the leading "v"
sudo git clone --depth 1 --branch "v$VERSION" \
  https://github.com/hephaestus-build/Hephaestus.git /opt/hephaestus
...
- Promotion happens **on sign-in**, is idempotent, and never demotes. If you forgot to set
  ...
  [auth runbook](https://github.com/hephaestus-build/Hephaestus/blob/main/docs/runbooks/auth-cutover.md#first-instance-admin-bootstrap).
...
### Running release and update checks

Open **Instance administration → Overview** to see the running identity and update-check result.
The [release image lock](./release-image-lock#runtime-identity-handoff) explains which identity is
reported and what a mismatch means. Unknown, failed and stale checks do not mean up to date.

Update checks are enabled by default in the server role when the build matches a stable release
identity supplied by the deployment lock. Successful results are cached for 24 hours; manual checks
respect the same cache and retry window. Failures retain the last successful result and retry with
exponential backoff, starting at 15 minutes and capped at 24 hours,
unless GitHub requests a longer wait. Restarting clears this history. Prereleases and builds
without matching deployment identity are not checked. An older published version is not a
downgrade recommendation.

The migration flag applies only to the named release; read every intervening entry in
[`MIGRATION.md`](https://github.com/hephaestus-build/Hephaestus/blob/main/MIGRATION.md) before upgrading.
A missing flag means unknown, not “no migrations.” Security relevance and backup/restore status are
not assessed by the checker. Release discovery neither verifies artifacts nor executes an upgrade;
use the signed-lock procedure below.

### Privacy and offline operation

Checks send an unauthenticated HTTPS GET to
`https://api.github.com/repos/hephaestus-build/Hephaestus/releases/latest`, with the static
`Hephaestus-release-check` user agent, GitHub API version/content-negotiation headers and the previous
response's ETag when available. No running version, digest, credentials, workspace data, configuration
or usage information is sent. GitHub sees the source IP and request time. The client does not use
workspace integration credentials or follow redirects.

For an air-gapped installation, set `HEPHAESTUS_RELEASE_CHECK_ENABLED=false` in `.env` **before
starting the application server**. To change an existing deployment, update the variable and recreate
`application-server` using the same Compose files and environment files as for installation. This
disables scheduled and manual checks without disabling local identity information. Set the variable
to `true` and recreate the service to resume checks.

Check failures appear only in instance administration. They do not affect application health or
readiness and do not notify workspace users.
...
[Back up the instance](./backup-restore), then check out the target release:
...
Compare your `.env` against the new release's `.env.example` for added variables. Complete the
intervening migration instructions linked above, including any PostgreSQL major-version upgrade,
before starting the new stack.

Verify the release lock, then deploy its images:

```bash
node scripts/prepare-release-lock.ts "v$VERSION" && (
  cd docker/self-host &&
  docker compose --env-file .env --env-file release-lock.env pull &&
  docker compose --env-file .env --env-file release-lock.env up -d
)
```

After deployment, compare `/actuator/info` with the selected lock on **each runtime role**, then
check `/actuator/health/readiness` on that role's management listener. Use internal access appropriate
to your [runtime topology](./runtime-roles); do not publish a management port for browser access.
The administration overview reports its server's identity, not a live inventory of the other roles.
A successful Compose command alone does not confirm that every role runs the intended release and
is ready.
