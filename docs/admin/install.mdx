...
published a newer one. A failed, rate-limited or never-completed check is shown as exactly that, never
as "up to date". The card links the newer release's notes and flags schema migrations when its notes
carry the flag — for that one release only; read every release note between yours and the newest
before upgrading. Nothing here verifies an artifact or performs an upgrade.

The server role asks a minute after start, then once a day; after a failure it retries from 15
minutes, doubling up to a day, or later if GitHub names a longer wait. **Check now** ignores the daily
cache but not a rate-limit window. A restart forgets the last answer. Deployments that do not run a
release — a commit channel, a local build — are not checked.

The check is one unauthenticated HTTPS GET to
`https://api.github.com/repos/hephaestus-build/Hephaestus/releases/latest` with the static user agent
`Hephaestus-release-check` and, on later checks, the previous answer's ETag. It carries no version,
credential, workspace data or configuration; GitHub sees the source IP and time. On an air-gapped host
set `HEPHAESTUS_RELEASE_CHECK_ENABLED=false` in `.env` and recreate `application-server`; the overview
then shows the identity and no check. Check failures reach only instance administration, never health
or readiness.
