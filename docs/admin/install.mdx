...
Read the target release's migration instructions before starting its images. Pre-1.0 upgrades may
require an intermediate release or manual preparation; the [Compatibility Policy](./compatibility-policy)
defines the guarantee and support window. Existing databases crossing the PostgreSQL 18/Liquibase
baseline transition must complete the [baseline synchronization runbook](./liquibase-baseline-runbook.md)
first. The commands below do not perform that preparation.

Complete the [runbook silent deployment checklist](./production-operations-runbook#silent-deployment-checklist)
through backup and staging validation, then acquire the tested release:
...
Before startup, compare `.env` with the release's `.env.example` and complete any manual steps in
[`MIGRATION.md`](https://github.com/hephaestus-build/Hephaestus/blob/main/MIGRATION.md), especially PostgreSQL
major-version changes. Return to **Validate then deploy** in the runbook for guarded startup.
The [compatibility policy](./compatibility-policy#upgrade-guarantee) defines skip-version support.

### Update checks

**Instance administration → Overview** shows the release this server reports from its lock
([runtime identity handoff](./release-image-lock#runtime-identity-handoff)) and whether GitHub has
published a newer one. A failed, rate-limited, disabled or never-completed check is shown as exactly
that, never as "up to date". For a newer release the card links its notes and the upgrade guide and
says whether that release carries schema migrations. The flag describes the newest release only:
read every release note between yours and it before upgrading. Nothing here verifies an artifact or
performs an upgrade.

The server asks GitHub a minute after start and then once a day. After a failure it retries after
15 minutes, doubling up to a day, or later when GitHub names a longer wait. **Check now** ignores the
daily cache, never delays the automatic retry, and only waits out a wait GitHub named. A restart
forgets the last answer. A commit channel or a local build runs no release and is never checked.

Each check is one unauthenticated HTTPS GET to
`https://api.github.com/repos/hephaestus-build/Hephaestus/releases/latest` with the static user agent
`Hephaestus-release-check` and, after the first answer, its ETag. It carries no version, credential,
workspace data or configuration; GitHub sees the source IP and the time. On an air-gapped host set
`HEPHAESTUS_RELEASE_CHECK_ENABLED=false` in `.env` and recreate `application-server`; the overview
then shows the identity and no check. A failed check reaches only instance administration, never
health or readiness.

## Rollback

Follow [failed-migration recovery](./production-operations-runbook#if-migration-fails) to restore the
previous release's database, secrets and signed lock. Reverting images alone is not a rollback:
never run an older release against a newer schema, reconstruct an old lock, or resolve mutable image tags.
