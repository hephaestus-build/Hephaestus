...
An existing GitHub installation stays bound to its workspace when the account is renamed. A new or
reinstalled GitHub App installation can reuse a matching-name workspace only when the workspace has
already recorded the same immutable GitHub organization identity. A PAT connection with a stored
token remains in PAT mode.

Personal-account workspaces and older organization workspaces that have not synchronized their
organization identity cannot establish that match from an account name alone. The installation gets
a separate workspace with an available slug; the existing workspace and its access remain unchanged.
Before migrating an organization from PAT mode, synchronize its organization identity and remove the
PAT credential when ready to switch. A name match never transfers ownership or private conversations.
If an old synchronized organization still holds a reused GitHub name, refresh that organization's
identity before retrying the installation; the conflicting identities are rejected rather than merged.
...
| Checks (`checks`) | Repository | Read | The state of the checks on a pull request's head, read with the pull request and kept current by `check_suite` events |
| Commit statuses (`statuses`) | Repository | Read | The same state from commit statuses, so non-Actions CI counts too, kept current by `status` events |
...
| `pull_request_review_thread` | Track review-thread resolution, and when it happened. |
| `check_suite` | Keep the head's check state current between syncs. |
| `status` | Keep the head's check state current from commit statuses. |
...
```json
{
  ...
  "default_events": [
    ...
    "pull_request_review_thread", "check_suite", "status", "issues", "issue_comment", "label",
    "milestone", "member", "repository", "discussion", "discussion_comment", "sub_issues",
...
An app created from an earlier manifest has to be brought up to the current one by hand under the
app's **Permissions & events**: subscribe to `check_suite` and `status` (no approval round), and
grant **Checks** and **Commit statuses** read if the app predates them (an approval round per
installation). Until an installation grants them, the head's check state reaches Hephaestus only
through the scheduled sync, which reads it with the pull request.
