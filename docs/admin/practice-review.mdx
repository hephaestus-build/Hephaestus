...
3. On **Practices → Review → When and where**, turn on **Start practice reviews**, then turn on at least
   one way in under **How reviews start** — **Start from work activity**, **Start requested reviews**, or
...
4. On **Practices → Review → How much**, set the **Workspace default** to **Review before sending** for the
   first test. Back on **When and where**, keep coverage at **All monitored repositories** and **All
   eligible linked members**, and leave **Send feedback** on.
5. Open a small pull or merge request whose author is an eligible linked member, then follow it in **Review
...
| **Review this now** on Review activity                     | The same people. Every member sees the button; unauthorized requests are refused            | Per-person hourly allowance, **and** the same per-work cooldown |
...
Two switches sit in front of the whole table. **Start from work activity** governs the first row;
**Start requested reviews** governs the other four together, so turning it off stops the button,
...
configured. Workspace coverage is not configured by an instance population flag; instance Silent Mode
is the separate emergency brake for external delivery. Variables, Compose forwarding, storage
requirements, queue metrics, retention, and fixed limits are in
...
- **Who authorizes release** — **Off**, **Review before sending**, or **Send automatically**. **Off** stops
  ...
  item for a workspace owner or administrator to approve or reject; the developer's own practice pages and
  the mentor are written regardless, because nobody but the developer reads them and only on request.
  **Send automatically** lets new feedback onto the work proceed without that decision, subject to the same
  delivery policy. Changing the setting never releases an existing proposal.
- **Coverage**, under **What gets reviewed**, is one workspace-wide intersection: all or selected monitored
  ...
  warns you when one is. The people side is linked human workspace membership — see
...
- **Send feedback**, under **Sending feedback**, controls whether feedback may leave Hephaestus. It stops
  comments on connected work and the mentor; it does not stop measurement, does not discard configuration, and
  does not blank the developer's own feedback pages here, which are not something leaving. Pausing is the
  immediate workspace brake. Feedback already approved but not sent when the pause takes effect is
  withheld permanently, just like automatic feedback, so resuming cannot release old work. Proposals that
  have not been decided stay in the approval queue; after resume, a workspace owner or administrator may
  still inspect and decide each one under the current policy.

Changing coverage, **Post feedback after merge**, or the workspace default autonomy invalidates admitted,
unsent automatic feedback — narrowing as much as widening. That feedback does not re-enter delivery; a
later review creates new feedback. Undecided proposals remain explicit workflow items and are checked
against current policy if a workspace owner or administrator later approves them. Changing the cooldown
does not advance the revision. Group and practice overrides are checked separately at delivery; they never
promote an existing proposal to automatic delivery.

### Who counts as a person

The people side of coverage contains linked human workspace members, and the count beside **People** on
the review screen is how many are in scope. A covered person must appear as eligible on the **Members**
screen. Manage a missing person in the connected organization, group, or team roster and let membership
sync pick them up; signing in to Hephaestus alone does not create workspace membership.

The person a practice evaluates must be covered. That is normally the pull-request or issue author;
reviewer practices use the reviewer. Outside contributors, unlinked reviewers, unidentified people, and
bots therefore remain outside coverage, and no feedback is prepared about them.
...
**Autonomy** is answered once for the **workspace**, and every group and every practice follows that answer
until you say otherwise. A group can override the workspace, and a practice can override its group:
...
workspace and again for each group. Read that line before changing anything; it is the fastest way to see
...
A review can make many model calls. These controls limit which reviews can start and what they can spend.
...
2. **The cooldown.** The minimum gap between reviews of the same work for the same occasion. New commits
   become eligible for one review at the latest commit after ten quiet minutes, or after sixty minutes
   from the first pending push. The cooldown can delay that review further. This is a grouping delay,
   not an hourly rate cap; queue and concurrency limits also affect when a review starts.
...
upstream of practice review — check the integration's sync. If it says the work is not showing on the
provider, nothing is misconfigured: a sync marked that work as gone and the reviews are being held. They
run themselves once a sync finds the work again, and are dropped if it does not within a week.
...
3. **Is a budget exhausted?** Administration → **AI usage** names which budget stopped new reviews and who
...
4. **Is practice review, or the workspace, switched off?** The switch is **Start practice reviews** on
   Administration → Practices → **Review → When and where** — *not* on the Workspace settings page. A
   workspace paused from Workspace settings runs nothing regardless of its AI configuration.
5. **Can anything still start a review?** Same screen, **How reviews start**. With both **Start from work
   activity** and **Start requested reviews** off, the workspace is on and nothing reaches it; the screen
...
7. **Is sending paused?** Same screen, **Sending feedback**. Reviews and in-app feedback continue, but no
   comments or mentor messages leave Hephaestus. Resuming does not release automatic feedback withheld by
   the pause.
...
10. **Is instance-wide silent mode engaged?** Instance admin → **Instance settings**. While it is on, no
   workspace's feedback leaves the instance — no comments on the work, no feedback in conversations and
   no email — while reviews still run and in-app feedback still reaches each developer's Practice
   profile. A banner says so on every instance-admin page.
