...
The promotion job summary names this boundary explicitly, including when a version check fails or
is skipped for a hold. It does not fetch host metrics: their textfile collector and authenticated
monitoring access belong to the host, not to the public application. Confirm a fresh successful
reconciliation and the expected release/channel commit on **each** participating host, then inspect
its Compose health and service logs. A matching frontend version alone says nothing about server,
worker, webhook, database, NATS, or integration readiness. Do not expose management endpoints to
make the public version check stand in for that operator verification.
