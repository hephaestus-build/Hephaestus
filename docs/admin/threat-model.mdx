...
accDescr: The diagram separates independently controlled systems, the operator edge, the Hephaestus application and data plane, and untrusted execution. Users and providers cross the edge into application and webhook roles. Verified events enter JetStream, services access PostgreSQL, sandboxes exchange inputs, results and model requests with the worker's sandbox gateway, and Git preparation containers fetch from the SCM provider.
  ...
  subgraph EXECUTION[Untrusted execution zone]
    ...
    G[Per-operation Git preparation container]
  end
  ...
  L -->|workspace tar down, result tar up, no provider key| S
  L -->|stored provider credential| M
  C -->|mirror volume, one fetch credential on stdin| G
  G -->|fetch only| X
  ...
  class U,V,X,M,S,G external
  class P,A,W,C,L,N,DB managed
```

Arrows show permitted logical flows, not network-level reachability. In particular, the container that carries the worker role — the application server in a monolith — joins each internal sandbox network; application route security remains part of that boundary. A sandbox's inputs arrive and its results leave through the worker's sandbox gateway, the same listener that hosts the LLM proxy; nothing on the worker's disk is mounted into a sandbox. Runtime-role separation reduces reachable surface but is not authorization; see [Runtime Roles](./runtime-roles).
...
| SB-3 — network exfiltration or cross-execution access | Create per-execution Docker resources. With internet access disabled, use an internal network with no external route and configure an unusable DNS resolver. | [`SandboxNetworkManager`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/SandboxNetworkManager.java), [`ContainerSecurityPolicy`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/ContainerSecurityPolicy.java) | [`SandboxNetworkManagerTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/SandboxNetworkManagerTest.java), [`DockerSandboxLiveTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/DockerSandboxLiveTest.java) | The job network can reach other application-server routes; those routes must independently authenticate and authorize sandbox-originated requests. `internetAccess=true` creates a normal bridge, not a hostname allow-list. |
| SB-4 — persistence after execution | Reconcile managed containers, networks and attempt volumes against durable job state, and Git preparation containers, mirrors and snapshot volumes against worker leases and repository monitors. | [`SandboxReconciler`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/SandboxReconciler.java), [`NativeGitVolumeReconciler`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/NativeGitVolumeReconciler.java) | [`SandboxReconcilerTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/SandboxReconcilerTest.java), [`NativeGitVolumeReconcilerTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/NativeGitVolumeReconcilerTest.java) | Host or daemon compromise is outside this control. |
| SB-5 — hostile repository content during Git preparation | Run every Git operation in its own container from the release-locked `git-preparation` image under the sandbox policy (SB-1), with the resource limits and deadline in [Practice review operations](./practice-review-operations.mdx#limits-you-should-know-about). Use a bridge network only while fetching and no network otherwise. The fetch credential enters on stdin for that one operation and an offline operation refuses one. A snapshot is a fresh bare repository with an empty hook template, `core.symlinks=false` and no remote configuration, measured against `GIT_MAX_SNAPSHOT_BYTES` before anything is written; the worker refuses snapshot archives carrying links, devices, FIFOs, traversal or absolute paths, and reads it into an attempt folder that is never mounted into a sandbox. | [`DockerNativeGitExecutor`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/DockerNativeGitExecutor.java), [`ContainerSecurityPolicy`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/ContainerSecurityPolicy.java), [`operation.ts`](https://github.com/hephaestus-build/Hephaestus/blob/main/docker/agents/git/operation.ts), [`GitRepositoryManager`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/integration/scm/domain/workdir/GitRepositoryManager.java) | [`DockerNativeGitExecutorLiveTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/DockerNativeGitExecutorLiveTest.java), [`ContainerSecurityPolicyTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/ContainerSecurityPolicyTest.java), [`GitRepositoryManagerTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/integration/scm/domain/workdir/GitRepositoryManagerTest.java), [`operation.test.ts`](https://github.com/hephaestus-build/Hephaestus/blob/main/docker/agents/git/operation.test.ts) | The mirror volume is shared by every operation on that repository on one worker; a file lock serialises a fetch against readers. Only the snapshot is measured against `GIT_MAX_SNAPSHOT_BYTES`: a fetch is not, so the mirror can grow past that bound on the worker's volume storage, and what the bound refuses is the snapshot taken from it. Git vulnerabilities remain possible; container isolation limits their impact but shares the host kernel (SB-1). When secret scanning is requested, a changed blob over the 8 MiB per-file budget refuses preparation rather than admitting an incomplete scan. |
| SB-6 — tampered or oversized workspace and result transport | Exchange inputs and results only through the sandbox gateway. A per-attempt credential registers one session; a workspace initializer container downloads the workspace tar exactly once against an advertised byte budget and refuses an over- or under-run; the runtime container then mounts `/workspace` read-only with separate writable `work/`, `out/`, `.pi/` and `.sessions/` volumes that are removed with the attempt. At most one result upload commits per session. The worker validates it in memory against the [workspace ABI archive contract](/contributor/agent/agent-workspace-abi#output-archive-validation); a wrong credential and an unknown session are indistinguishable. | [`SandboxAttemptLauncher`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/SandboxAttemptLauncher.java), [`DockerAttemptWorkspace`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/DockerAttemptWorkspace.java), [`SandboxGatewaySessions`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/agent/gateway/SandboxGatewaySessions.java), [`SandboxWorkspaceController`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/agent/gateway/SandboxWorkspaceController.java), [`SandboxOutputArchive`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/agent/runtime/SandboxOutputArchive.java), [`gateway-init.ts`](https://github.com/hephaestus-build/Hephaestus/blob/main/docker/agents/pi/gateway-init.ts) | [`SandboxGatewaySessionsTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/agent/gateway/SandboxGatewaySessionsTest.java), [`SandboxWorkspaceControllerTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/agent/gateway/SandboxWorkspaceControllerTest.java), [`SandboxGatewayHandshakeInterceptorTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/agent/gateway/SandboxGatewayHandshakeInterceptorTest.java), [`SandboxOutputArchiveTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/agent/runtime/SandboxOutputArchiveTest.java) | The initializer runs the agent image under the same policy as the runtime; what makes it trusted is that only `gateway-init.ts` runs before the runtime container exists. The gateway is plain HTTP on the internal sandbox network; a compromised daemon or host can read the transfer. |
...
| EG-4 — disclosure of stored integration credentials | Encrypt credential fields mapped through the converter with AES-GCM and bind their ciphertext to the owning row and purpose. | [ADR 0014](https://github.com/hephaestus-build/Hephaestus/blob/main/docs/decisions/0014-per-row-aes-gcm-aad-binding.md), [`EncryptedStringConverter`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/core/security/EncryptedStringConverter.java) | [`EncryptedStringConverterTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/core/security/EncryptedStringConverterTest.java), [`EncryptionContextTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/integration/core/connection/EncryptionContextTest.java) | Application or encryption-key compromise exposes decryptable values. Rotate credentials upstream after exposure; [#543](https://github.com/hephaestus-build/Hephaestus/issues/543) tracks encryption-key versioning and rotation. |
...
| ID and threat | Enforced control | Enforcement | Verification | Residual risk |
| --- | --- | --- | --- | --- |
| TN-1 — object-ID substitution or role escalation | Resolve the authenticated account's workspace membership and role before protected workspace operations. | [`WorkspaceSecurityExpressions`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/workspace/authorization/WorkspaceSecurityExpressions.java) | [`WorkspaceAccessServiceTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/workspace/authorization/WorkspaceAccessServiceTest.java), [`CrossTenantIsolationIntegrationTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/workspace/CrossTenantIsolationIntegrationTest.java) | This control applies only where membership is checked; a user-supplied identifier is not proof of ownership. |
