...
accDescr: The diagram separates independently controlled systems, the operator edge, the Hephaestus application and data plane, and untrusted sandbox execution. Users and providers cross the edge into application and webhook roles. Verified events enter JetStream, services access PostgreSQL, sandboxes exchange inputs, results and model requests with the worker's sandbox gateway, and the server and worker fetch repository mirrors from the SCM provider in-process.
  ...
  A -->|purpose-specific client, mirror fetch| X
  C -->|purpose-specific client, mirror fetch| X
...
| SB-4 — persistence after execution | Reconcile managed containers, networks and attempt volumes against durable job state, and sweep the attempt folders and repository snapshots of ended attempts from worker storage. | [`SandboxReconciler`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/SandboxReconciler.java), [`JobEvidenceFiles`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/agent/context/JobEvidenceFiles.java) | [`SandboxReconcilerTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/agent/sandbox/docker/SandboxReconcilerTest.java), [`JobEvidenceFilesTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/agent/context/JobEvidenceFilesTest.java) | Host or daemon compromise is outside this control. |
| SB-5 — hostile repository content during Git preparation | Parse repository bytes with Eclipse JGit inside the application process: memory-safe, with no `git` binary, and no hook is ever executed. The fetch credential is handed to JGit in memory for that one fetch and never written to the mirror's configuration; `http.followRedirects=false`. A snapshot is a fresh repository under the fabric root fetched from the local mirror with `core.symlinks=false`, no remote and no credentials, measured against `GIT_MAX_SNAPSHOT_BYTES` before anything is written and refused whole when over; submodule entries and unsafe paths are excluded and recorded as limitations. The checkout is read into an attempt folder that is never mounted into a sandbox and reaches the sandbox only as the workspace tar (SB-6). | [`GitRepositoryManager`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/integration/scm/domain/workdir/GitRepositoryManager.java), [`GitRepositoryLockManager`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/integration/scm/domain/workdir/GitRepositoryLockManager.java), [`JobEvidenceFiles`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/main/java/de/tum/cit/aet/hephaestus/agent/context/JobEvidenceFiles.java) | [`GitRepositoryManagerTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/integration/scm/domain/workdir/GitRepositoryManagerTest.java), [`JobEvidenceFilesTest`](https://github.com/hephaestus-build/Hephaestus/blob/main/server/application/src/test/java/de/tum/cit/aet/hephaestus/agent/context/JobEvidenceFilesTest.java) | A JGit vulnerability runs with the privileges of the role that fetched — the server for commit ingestion, the worker for review preparation — not inside a sandbox. Only the snapshot is measured against `GIT_MAX_SNAPSHOT_BYTES`: a fetch is not, so the mirror can grow past that bound under the container's fabric root, and what the bound refuses is the snapshot taken from it. The mirror is shared by every operation on that repository in one container; an in-process lock serialises a fetch against readers. |
