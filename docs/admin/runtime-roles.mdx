...
read `—` in that column. Git needs no such row: it runs in-process through JGit in whichever role
does the work, against that container's own mirrors under `hephaestus.fabric.root`. Commit ingestion
runs on the server role, review preparation on the worker role, and a server pod whose worker role is
enabled uses its one root for both; the webhook role has no Git at all.
