...
| `hephaestus.git.image` | `ghcr.io/hephaestus-build/git-preparation:<app version>` | same | — | env default (`HEPHAESTUS_IMAGE_GIT_PREPARATION`); the shipped Compose files require the release lock's digest |
...
read `—` in that column. Native Git follows the same rule: a pod with the worker role runs Git
preparation and commit ingestion in its own `git-preparation` containers, a server pod whose worker
role is turned off dispatches each Git operation over the control channel to a connected worker —
placed by hashing the repository id over the connected workers, so a repository keeps its worker
while that worker's session lives — and the webhook role has no Git at all, so the `git.image` row
reads `—` there too.
...
- [**ADR 0005**](https://github.com/hephaestus-build/Hephaestus/blob/main/docs/decisions/0005-two-role-runtime-via-conditional-on-property.md) — original two-role design.
- [**ADR 0008**](https://github.com/hephaestus-build/Hephaestus/blob/main/docs/decisions/0008-webhook-runtime-role.md) — webhook role rationale.
- [**ADR 0009**](https://github.com/hephaestus-build/Hephaestus/blob/main/docs/decisions/0009-worker-runtime-substrate-wss-control-channel.md) — worker substrate + WSS control channel.
- [**ADR 0025**](https://github.com/hephaestus-build/Hephaestus/blob/main/docs/decisions/0025-agent-job-queue-on-postgresql.md) — agent job queue moved off NATS onto PostgreSQL.
