...
All three default-on means `vp run dev:server` starts a working monolith.
...
does differ by role. The whole `core.auth` module carries `@ConditionalOnServerRole`, so setting
`hephaestus.runtime.server.enabled=false` drops it; the `worker` overlay additionally excludes
`OAuth2ResourceServerAutoConfiguration` outright, so a worker-only pod never depends on user-auth
wiring at boot. The `webhook` overlay does not carry that exclusion and relies on the role gate alone.
