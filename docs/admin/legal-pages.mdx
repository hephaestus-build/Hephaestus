...
If your deployment is the canonical TUM / AET deployment, set `LEGAL_PROFILE=tumaet` in your environment. No other configuration is needed — the `tumaet` profile is baked into the webapp image and contains the audited texts matching the [DSMS record](./dsms) on file at TUM.
...
Third-country transfers, when you enable an external LLM provider (Microsoft Azure OpenAI, OpenAI, or Anthropic), must be covered under Chapter V GDPR. OpenAI and Microsoft are currently DPF-certified; for those providers the primary safeguard is the Commission adequacy decision under Art. 45(3) GDPR in relation to the DPF, with Standard Contractual Clauses under Art. 46(2)(c) GDPR contracted as a fall-back. Anthropic is not on the U.S. Department of Commerce DPF active list and relies on Standard Contractual Clauses Module 2 under Art. 46(2)(c) GDPR alone.
