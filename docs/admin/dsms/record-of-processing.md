...
```text
Repository-activity artefacts authored by the contributor in the connected Git repositories (pull/merge requests, issues, code reviews, review comments, commit metadata and, when an enabled practice declares it, a bounded repository tree), AI guidance-assistant conversations, product-feedback messages and optionally shared page paths, survey answers and dismissals, selected Outline project documents, and Slack integration data when enabled (Slack IDs, Slack identity links, App Home privacy choices, Hephaestus DM mentor messages, and new messages in administrator-activated monitored Slack channels).
```
